"""
V3.E Seed Ensemble -- Inference wrapper for multi-seed ensemble models.
...
class SnapshotEnsemble(nn.Module):
    ...
    All models share the same architecture (WaveNetGRUWorldModel),
    ...
    def __init__(self, snapshot_paths: list = None, top_k: int = ENSEMBLE_TOP_K,
        ...
        if snapshot_paths is None:
            ...
            snapshot_paths = sorted(snap_dir.glob("v3_1e_seed_*_best.pt"))
            if not snapshot_paths:
                # Fallback to final models (backward compatibility)
                snapshot_paths = sorted(snap_dir.glob("v3_1_seed_*.pt"))
        ...
        if snapshot_ics and len(snapshot_paths) > top_k:
            # Build (path, ic) pairs, sort by IC descending
            ranked = []

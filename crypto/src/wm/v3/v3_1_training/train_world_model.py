"""
V3.1 World Model Trainer -- WaveNet-GRU-RSSM + XD Anti-Memorization

Architecture: WaveNet-style Gated Dilated Convolutions + GRU + RSSM
Version: v3_1_wavenet_gru_rssm_antifragile

Supports --features 13|18|30|37:
  - 13: Legacy features only (base_dim == input_dim, no XD paths)
  - 18: Extended features
  - 30: All base features
  - 37: Full features with XD cross-asset enrichment
...
class CheckpointManager:
    ...
    def __init__(self, save_dir: Path, keep_top_k: int = 3, prefix: str = "v3_1_f18"):
    ...
    def save(self, model, optimizer, scaler, epoch, val_loss,
        ...
        state = {
            ...
            "n_features": n_features or getattr(self, "_expected_n_features", None),
...
def train_world_model(n_features: int = 18, use_revin: bool = False):
    """
    Anti-fragile training loop for V3.1 WaveNet-GRU-RSSM.
    ...
    ckpt_prefix = f"v3_1_{feat_tag}"
    log_path = setup_logging(LOG_DIR, f"v3_1_{feat_tag}_train")
    ...
    print("=" * 70)
    print("  V3.1 WORLD MODEL TRAINER (WaveNet-GRU-RSSM | Anti-Fragile)")
    print("=" * 70)
    ...
    print_antifragile_header("V3.1", af_config)
...
if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description="V3.1 World Model Trainer")

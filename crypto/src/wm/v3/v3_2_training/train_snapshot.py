...
def save_master_checkpoint(seeds, completed, seed_metrics, feat_tag: str = "f18"):
    ...
    state = {
        ...
        "version": "v3_2e_multi_seed_ensemble",
    }
    torch.save(state, ENSEMBLE_MODEL_DIR / f"v3_2e_{feat_tag}_master.pt")
...
def train_single_seed(
    ...
    start_epoch = 0
    ckpt_path = ENSEMBLE_MODEL_DIR / f"v3_2e_{feat_tag}_seed_{seed_idx}_latest.pt"
    if ckpt_path.exists():
    ...
    autopsy_path = ENSEMBLE_MODEL_DIR / f"v3_2e_seed_{seed_idx}_autopsy.jsonl"
        ...
        if (epoch + 1) % WM_VAL_EVERY == 0 or epoch == WM_TOTAL_EPOCHS - 1:
            ...
            if (epoch + 1) % af_config.shuffled_ic_every == 0:
                ...
                if shuffled_ic > best_shuffled_ic:
                    ...
                    }, ENSEMBLE_MODEL_DIR / f"v3_2e_{feat_tag}_seed_{seed_idx}_best.pt")
            ...
            state = {
                ...
                "seed": seed,
                "seed_idx": seed_idx,
                "version": "v3_2e_seed_training",
                "n_features": INPUT_DIM,
            }
            torch.save(state, ENSEMBLE_MODEL_DIR / f"v3_2e_{feat_tag}_seed_{seed_idx}_latest.pt")

            # Keep top-3 epoch checkpoints
            ep_path = ENSEMBLE_MODEL_DIR / f"v3_2e_{feat_tag}_seed_{seed_idx}_epoch_{epoch+1}.pt"
    ...
    final_path = ENSEMBLE_MODEL_DIR / f"v3_2_{feat_tag}_seed_{seed_idx}.pt"
    torch.save({
        "model_state_dict": ema_model.state_dict(),
        "revin_state_dict": revin.state_dict(),
    }, final_path)
    ...
    latest_ckpt = ENSEMBLE_MODEL_DIR / f"v3_2e_{feat_tag}_seed_{seed_idx}_latest.pt"
    ...
    for ep_ckpt in ENSEMBLE_MODEL_DIR.glob(f"v3_2e_{feat_tag}_seed_{seed_idx}_epoch_*.pt"):
        try:
...
def train_ensemble(n_features: int = 18):
    ...
    log_path = setup_logging(LOG_DIR, f"v3_2e_{feat_tag}_ensemble_train")
    ...
    print("=" * 70)
    print("  V3.2.E MULTI-SEED ENSEMBLE TRAINER")
    print("=" * 70)
    ...
    master_path = ENSEMBLE_MODEL_DIR / f"v3_2e_{feat_tag}_master.pt"
    ...
    existing_seeds = sorted(ENSEMBLE_MODEL_DIR.glob(f"v3_2_{feat_tag}_seed_*.pt"))

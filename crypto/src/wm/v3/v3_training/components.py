...
class WaveNetBlock(nn.Module):
    ...
    def forward(self, x: torch.Tensor):
        ...
        """
        # Gated activation: tanh(filter) * sigmoid(gate)
        # Force fp32 for the gated activation — tanh/sigmoid backward overflows
        # fp16 when filter activations exceed ~4.0 (1/cosh^2 * sigmoid gradient
        # chain exceeds fp16 max 65504). This causes GN:inf from epoch ~20 onward
        # as WaveNet weights warm up and push activations into the saturation zone.
        ...
        h = h.to(x.dtype)  # back to fp16 for downstream ops
        h = self.dropout(h)
...
class MultiScaleAggregator(nn.Module):
    ...
    NORMALIZATION (Phase 14.7 fix): scale skip-sum by 1/sqrt(N_blocks),
    apply post-aggregation RMSNorm. Without this, summing N unnormalized
    skip tensors produces gradients that grow O(N) → grad explosion in
    audit B=32 probe (V3 grad_max=6.67e12 pre-fix).
    """
    ...
    def __init__(self, channels: int, out_channels: int):
        ...
        # Post-aggregation normalization to prevent grad explosion
        # caused by summing N unnormalized skip tensors.
    ...
    def forward(self, skips: list) -> torch.Tensor:
        ...
        Returns:
            out: [B, T, C_out]
        """
        # Scale by 1/sqrt(N) to keep variance bounded as stack depth grows.
        n_blocks = len(skips)
        scale = (n_blocks ** 0.5) if n_blocks > 1 else 1.0
        # fp32 for skip aggregation — sum of N WaveNet skip tensors overflows fp16
        aggregated = sum(s.float() for s in skips) / scale  # [B, C, T] in fp32
        out = self.proj(aggregated.to(skips[0].dtype))  # [B, C_out, T] back to AMP dtype
        out = out.transpose(1, 2)  # [B, T, C_out]
        return self.post_norm(out)  # post-aggregation RMSNorm

"""
V3.D NCL Diversity Validation -- Per-head IC + ensemble IC + diversity metrics

Validates the V3.D DiversityWorldModel checkpoint from NCL_MODEL_DIR.
...
def load_model(model_path=None, use_revin: bool = False):
    ...
    if model_path is None:
        ckpt_path = NCL_MODEL_DIR / "v3d_ncl_best_ema.pt"
        if not ckpt_path.exists():
            ckpt_path = NCL_MODEL_DIR / "v3d_wm_best_ema.pt"
        if not ckpt_path.exists():
    ...
    print(f"  [OK] V3.D loaded  |  heads={K}  |  NCL lambda={model.ncl_lambda}")
    ...
    for feats, targets, asset_idx, asset_name in tqdm(segments, desc="  Assets", leave=True):
        ...
        for k in range(K):
            ...
            for h in REWARD_HORIZONS:
                ...
                # reals are the same as ensemble reals
                r = np.concatenate(a_ens_reals[h])
...
def compute_diversity_metrics(head_preds, ensemble_reals, K):
    ...
    """
    # Concatenate all predictions for each head at horizon=1
    preds_by_head = []
    for k in range(K):
        p = np.concatenate(head_preds[k][1])
        preds_by_head.append(p)

    # Clip to same length (should be identical, but guard for edge cases)
    ...
    # Per-timestep head std (diversity at each bar)
...
def print_report(asset_results, ensemble_preds, ensemble_reals, head_preds,
    ...
    print(f"\n{'='*70}")
    print(f"  V3.D NCL DIVERSITY VALIDATION")
    ...
    # Per-head IC (global, horizon=1 focus)
    ...
    if all_pass:
        ...
        print(f"  V3.D NCL model is validated.")
    else:
        print(f"  VERDICT: GATE(S) FAILED")
        print(f"  V3.D NCL model needs more training or tuning.")
    print(f"  {'='*40}")
...
def main():
    parser = argparse.ArgumentParser(description="V3.D NCL Diversity Validation")
    ...
    print(f"\n{'='*70}")
    print(f"  V3.D NCL DIVERSITY VALIDATOR")

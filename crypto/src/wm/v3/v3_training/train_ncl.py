"""
V3.D Diversity World Model Trainer -- Multi-Head NCL (Anti-Fragile Edition)

Architecture: V3 backbone (WaveNet-GRU-RSSM) + K parallel return heads with NCL
Version: v3d_diversity_ncl_antifragile

Key differences from train_world_model.py:
  - Uses DiversityWorldModel instead of WaveNetGRUWorldModel
  - Logs NCL penalty in epoch summary
  - Can optionally load V3 backbone weights (--load-backbone)
  - Saves checkpoints as v3d_wm_*.pt
  - Uses same LR schedule, augmentation, and anti-fragile framework as V3

Anti-fragile training features (inherited from V3):
...
def train_diversity_model(load_backbone: str = None, freeze_backbone: bool = False, use_revin: bool = False, n_features: int = None):
    """
    Anti-fragile training loop for V3.D Diversity World Model.

    Same framework as V3 trainer but with:
    - DiversityWorldModel (K parallel return heads + NCL)
    - NCL penalty logging
    - Optional V3 backbone loading
    - v3d_* checkpoint naming

    Args:
        load_backbone: path to V3 checkpoint to load backbone from (optional)
        freeze_backbone: if True, freeze backbone and only train diversity heads
    """
    log_path = setup_logging(LOG_DIR, "v3d_train")
    ...
    print("=" * 70)
    print("  V3.D DIVERSITY WORLD MODEL TRAINER (Multi-Head NCL | Anti-Fragile)")
    print("=" * 70)
    print(f"  Device:       {DEVICE}")
    print(f"  Platform:     {'Windows' if IS_WINDOWS else 'Linux/Mac'}")
    print(f"  Features:     {INPUT_DIM}")
    print(f"  Horizons:     {REWARD_HORIZONS}")
    print(f"  Architecture: WaveNet-GRU-RSSM | GRU={GRU_HIDDEN_DIM}, "
          f"TCN={TCN_CHANNELS}, dilations={TCN_DILATIONS}")
    ...
    print_antifragile_header("V3.D", af_config)
    ...
    ckpt_path = NCL_MODEL_DIR / "v3d_wm_latest.pt"
    if ckpt_path.exists():
    ...
    autopsy_path = NCL_MODEL_DIR / f"v3d_autopsy.jsonl"
        ...
        if (epoch + 1) % WM_VAL_EVERY == 0 or epoch == DIVERSITY_TOTAL_EPOCHS - 1:
            ...
            if (epoch + 1) % af_config.shuffled_ic_every == 0:
                ...
                if shuffled_ic > best_shuffled_ic:
                    ...
                    torch.save(best_ema_state, NCL_MODEL_DIR / f"v3d{revin_tag}_wm_best_ema.pt")
            ...
            torch.save(state, NCL_MODEL_DIR / "v3d_wm_latest.pt")
            torch.save(model.state_dict(), NCL_MODEL_DIR / "v3d_wm_weights.pt")

            ep_path = NCL_MODEL_DIR / f"v3d_wm_epoch_{epoch+1}.pt"
    ...
    # -- Final Report ----------------------------------------------------------
    print("\n" + "=" * 70)
    print("  V3.D DIVERSITY TRAINING COMPLETE (Anti-Fragile)")
    print("=" * 70)
    ...
    print(f"  EMA weights:         {NCL_MODEL_DIR / f'v3d{revin_tag}_wm_best_ema.pt'}")
    print(f"  Latest checkpoint:   {NCL_MODEL_DIR / 'v3d_wm_latest.pt'}")

    if not gate_passed:
        print("\n  [WARN] Diversity model did not pass validation gate.")
        print("  Do NOT proceed to agent training until gate criteria are met.")
    else:
        print("\n  [OK] V3.D Diversity model ready for downstream use.")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="V3.D Diversity World Model Trainer")
    parser.add_argument(
        "--load-backbone", type=str, default=None,
        help="Path to V3 checkpoint to initialize backbone weights from"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--features", type=int, choices=[13, 18, 30, 34, 37, 41], default=None,

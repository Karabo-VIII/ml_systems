"""
V3 World Model Trainer -- WaveNet-GRU-RSSM Edition (Anti-Fragile)

Architecture: WaveNet-style Gated Dilated Convolutions + GRU + RSSM
Version: v3_wavenet_gru_rssm_antifragile
...
# expandable_segments=True is the V25-era fix (commit 134039d). See V4 trainer
# for full receipt + V4 OOM at v4_validate_2026_05_10c.log.
...
# --clean flag selects stripped WaveNet-Direct model (no GRU/RSSM/recon/dream)
...
if _pre_args.frontier:
    from world_model_clean import WaveNetCleanModel as WaveNetGRUWorldModel, count_parameters
else:
...
try:
    ...
    def add_meta_args(parser):
        parser.add_argument("--meta", type=str, default="",
                             help="NO-OP for V3 (only V25 implements meta-learners)")
        parser.add_argument("--meta-distill-alpha", type=float, default=0.0,
                             help="NO-OP for V3 (paired with --meta)")
...
def collate_fn(batch):
    ...
    # Forward-regime labels (V3_FORWARD_REGIME flag; absent when flag is OFF)
...
def _targets_to_device(targets, device):
    ...
    Plain tensors -> .to(device). A nested dict (the V3_FORWARD_REGIME
    `forward_regime_labels` = {"bear","trend","move"}) is recursed one level so its
    inner tensors land on GPU. When V3_FORWARD_REGIME is OFF there is NO nested dict,
...
class CheckpointManager:
    ...
    def __init__(self, save_dir: Path, keep_top_k: int = 3, prefix: str = "v3"):
    ...
    def load_latest(self, model, optimizer, scaler, ema_model=None, revin=None):
        ...
        try:
            ...
            model_nf = model.obs_encoder[0].in_features - model.asset_embedding.embedding_dim
            if ckpt_nf is not None and ckpt_nf != model_nf:
                print(f"  [WARN] Checkpoint n_features={ckpt_nf} != model n_features={model_nf}. Starting fresh.")
                return 0, float("inf"), 0, False, -float("inf"), 0
            ...
            print(f"  Resumed at epoch {start_epoch}, best_val={best_val:.4f}, "
                  f"best_shIC={best_shic:.4f}, patience={patience}, shic_decline={shic_decline}")
...
def validate(model, val_loader, revin=None):
    ...
    metrics = {"rec": [], "kl": [], "kl_raw": [], "regime": [], "regime_acc": [], "total": []}
    for h in REWARD_HORIZONS:
        metrics[f"ret_{h}"] = []
...
def check_gate(val_metrics: dict, shuffled_ic: float = None,
    ...
    # Clean variant has no RSSM -> no rec/kl to gate on. Skip those checks.
    _is_clean = _pre_args.clean
    if not _is_clean and rec > GATE_REC_MSE_MAX:
        reasons.append(f"Rec MSE={rec:.4f} > {GATE_REC_MSE_MAX}")
    if ic < GATE_IC_MIN:
        reasons.append(f"IC={ic:.4f} < {GATE_IC_MIN}")
    if not _is_clean and kl < GATE_KL_MIN:
        reasons.append(f"KL={kl:.4f} too low (collapse)")
    if not _is_clean and kl > GATE_KL_MAX:
...
def train_world_model(use_revin: bool = False, n_features: int = 37, args=None):
    ...
    # -- VSN CLI flag -> env (MUST precede model construction) ------------------
    # The model reads V3_VSN at __init__ to decide whether to build self.vsn.
    # Setting the env from --vsn here (BEFORE WaveNetGRUWorldModel(...) below)
    # ensures the CLI path constructs the VSN; otherwise model.vsn stays None and
    # the later "model.vsn.parameters()" report would crash. The env-var path
    # (V3_VSN=1 already set) is unaffected.
    if args is not None and getattr(args, "vsn", False):
        os.environ["V3_VSN"] = "1"

    log_path = setup_logging(LOG_DIR, f"v3_f{n_features}{run_tag_str}_train")
    ...
    print("=" * 70)
    print("  V3 WORLD MODEL TRAINER (WaveNet-GRU-RSSM | Anti-Fragile)")
    print("=" * 70)
    ...
    print(f"  GRU Hidden:   {GRU_HIDDEN_DIM} x {GRU_NUM_LAYERS} layers")
    print(f"  RSSM Latent:  {RSSM_LATENT_DIM}x{RSSM_CLASSES} = {FLAT_DIM}")
    print_antifragile_header("V3", af_config)
    ...
    # -- Initialize Model ------------------------------------------------------
    model = WaveNetGRUWorldModel(input_dim=input_dim).to(DEVICE)
    ...
    # -- VSN flag report (V3_VSN, combinable with V3_FORWARD_REGIME) -----------
    # Model was already built with vsn wired in (env var read at __init__ time).
    # We confirm the flag here so the training log is self-describing.
    _use_vsn = os.environ.get("V3_VSN", "0") == "1"
    if args is not None and getattr(args, "vsn", False):
        os.environ["V3_VSN"] = "1"
        _use_vsn = True
    if _use_vsn:
        vsn_params = sum(p.numel() for p in model.vsn.parameters()) if model.vsn is not None else 0
        print(f"  [V3_VSN] Variable Selection Network ENABLED  "
        ...
        print(f"  [V3_VSN] VSN disabled (default). Set V3_VSN=1 or --vsn to enable.")

    # -- Forward-regime head (V3_FORWARD_REGIME flag, 2026-06-10) -------------
    # Activated by env flag V3_FORWARD_REGIME=1 or CLI --forward-regime (default OFF).
    # When OFF: no attachment, no label computation, base path byte-for-byte unchanged.
    # When ON:  attach head to model + ema_model, pre-compute forward labels per segment,
    #           pack them into targets via collate_fn -> get_loss adds 0.10 * aux_loss.
    _use_forward_regime = os.environ.get("V3_FORWARD_REGIME", "0") == "1"
    if args is not None and getattr(args, "forward_regime", False):
        _use_forward_regime = True
    if _use_forward_regime:
        ...
        # Labels stored on segment dicts; AntifragileDataset.__getitem__ picks them up
        # by key (fwd_bear/fwd_trend/fwd_move) just like regime_label.
        # Close proxy: cumprod(1 + target_return_1) -- sufficient for relative-price labels.
        print("  [V3_FORWARD_REGIME] Pre-computing forward labels on all segments ...")
        _all_label_segs = all_segments
        for _seg in _all_label_segs:
            ...
            if _ret1 is None:
                ...
                continue
            _cret = np.clip(_ret1.astype(np.float64), -0.99, 10.0)
        ...
        print(f"  [V3_FORWARD_REGIME] Labels ready on {len(_all_label_segs)} segments "
    ...
    if getattr(args, "autopsy", False):
        try:
            ...
            autopsy = AutopsyMode(
                model,
                log_dir=Path(__file__).resolve().parents[4] / "logs" / "v3",
                run_tag=f"v3_f{input_dim}",
    ...
    # -- Checkpoint ------------------------------------------------------------
    ckpt_mgr = CheckpointManager(BASE_MODEL_DIR, keep_top_k=3, prefix=f"v3_f{n_features}{revin_tag}{run_tag_str}")
    ...
    nan_count = 0
    max_nan_per_epoch = 100  # WaveNet fp16 produces occasional NaN; 20 was too aggressive (aborted at 1% of 2000 steps)

    # -- Feature Autopsy (non-console diagnostics) ----------------------------
    autopsy_path = LOG_DIR / f"v3_f{n_features}{revin_tag}{run_tag_str}_autopsy_{log_path.stem.split('_train_')[-1]}.jsonl"
    ...
    if _pre_args.clean:
        print(f"  VIB:          z_dim={VIB_Z_DIM}, kl_weight_max={VIB_KL_WEIGHT}, anneal={KL_ANNEAL_EPOCHS}ep")
        print(f"  Defenses:     seq_shuffle={SEQ_SHUFFLE_PROB}, mask {WM_MASK_RATIO_START}->{WM_MASK_RATIO_END}, mixup, jitter")
    else:
    ...
    for epoch in range(start_epoch, _max_epochs):
        ...
        epoch_keys = ["total", "rec", "kl", "kl_raw", "regime", "regime_acc"] + [f"ret_{h}" for h in REWARD_HORIZONS]
        ...
        for step in pbar:
            ...
            # forward_regime_labels is a nested dict -> skipped (noise is for the
            # return targets only; cannot .dtype a dict).
            ...
            if upgrade_ctx is not None and upgrade_ctx.use_sam:
                ...
                grad_norm = torch.nn.utils.clip_grad_norm_(clip_params, WM_GRAD_CLIP)
            ...
            if autopsy is not None:
                try:
                    autopsy.step(step, loss_components={
                        ...
                        "rec": float(loss_dict.get("rec", 0.0)),
                        "kl": float(loss_dict.get("kl", 0.0)),
        ...
        #   Full (7-elem):  [rec, kl, ret_1, ret_4, ret_16, ret_64, regime]
        #   Clean (5-elem): [ret_1, ret_4, ret_16, ret_64, regime]  (no rec/kl)
        with torch.no_grad():
        ...
        if (epoch + 1) % WM_VAL_EVERY == 0 or epoch == WM_TOTAL_EPOCHS - 1:
            ...
            if (epoch + 1) % af_config.shuffled_ic_every == 0:
                ...
                if shuffled_ic > best_shuffled_ic:
                    ...
                    torch.save(best_ema_dict, BASE_MODEL_DIR / f"v3_f{n_features}{revin_tag}{run_tag_str}_wm_best_ema.pt")
    ...
    print(f"  Weights saved:    {BASE_MODEL_DIR / f'v3_f{n_features}{revin_tag}{run_tag_str}_wm_best_ema.pt'}")
    print(f"  Checkpoint:       {BASE_MODEL_DIR / 'v3_wm_latest.pt'}")
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="V3 World Model Trainer")
    parser.add_argument("--seed", type=int, default=None, help="Random seed for reproducibility")
    parser.add_argument("--max-epochs", type=int, default=None,
                        help="Override WM_TOTAL_EPOCHS for short validation runs (default: settings.WM_TOTAL_EPOCHS)")
    ...
    parser.add_argument("--features", type=int, choices=sorted(SUPPORTED_FEATURE_COUNTS_V3), default=13,
                        help="Number of features: 13 (legacy), 18 (extended), 30 (all base), 37 (full+XD)")
    parser.add_argument("--clean", action="store_true",
                        help="Use stripped WaveNet-Direct (no GRU/RSSM/recon/dream); consumed at import time")
    parser.add_argument("--vsn", action="store_true",
                        help="[V3_VSN] Variable Selection Network: per-timestep learnable "
                             "feature gate g_t=sigmoid(W*x_t) applied before obs_encoder. "
                             "Causal (per-timestep only). ~input_dim^2 extra params. "
                             "Equivalent to setting env var V3_VSN=1. "
                             "Combinable with --forward-regime (both ON = full world-class candidate). "
                             "OFF by default: base input path byte-for-byte unchanged.")
    parser.add_argument("--forward-regime", action="store_true",
                        help="[V3_FORWARD_REGIME] Attach forward-regime/move-onset heads: "
                             "3 small MLP heads (bear/trend/move-onset) on feat=cat(h_seq,z_post). "
                             "Supervised by forward labels from regime_targets.py (NaN tail masked). "
                             "Aux loss weight=0.10 (additive; base loss unchanged when OFF). "
                             "Equivalent to setting env var V3_FORWARD_REGIME=1. "
                             "OFF by default: no attachment, no label computation.")

"""
V3 World Model Validation — Comprehensive Diagnostics Suite (WaveNet-GRU-RSSM)
...
class WorldModelValidator:
    """Comprehensive diagnostics for V3 WaveNet-GRU World Model."""

    def __init__(self, model_path: Path = None, use_revin: bool = False,
                 n_features: int = None, run_tag: str = ""):
        # n_features defaults to settings.INPUT_DIM if unspecified.
        self.n_features = n_features if n_features is not None else INPUT_DIM
        self.model = WaveNetGRUWorldModel().to(DEVICE)
        self.model_name = "unknown"
        self.revin = RevIN(num_features=INPUT_DIM).to(DEVICE) if use_revin else None
        if self.revin is not None:
            self.revin.eval()
        self.revin_tag = "_revin" if use_revin else ""
        self.run_tag_str = f"_{run_tag}" if run_tag else ""
        self._load_model(model_path)
        self.model.eval()

    def _load_model(self, model_path: Path):
        # Cohort-harmonized 2026-05-03: ckpts include f{n_features} + run_tag
        # to match the new trainer write pattern. Falls back to legacy bare
        # name only if the new path is missing (graceful pre-rename support).
        prefix = f"v3_f{self.n_features}{self.revin_tag}{self.run_tag_str}"
        if model_path is None:
            model_path = BASE_MODEL_DIR / f"{prefix}_wm_best_ema.pt"
        if not model_path.exists():
            model_path = BASE_MODEL_DIR / f"{prefix}_wm_latest.pt"
        if not model_path.exists():
            # Legacy fallback (pre-rename ckpts).
            legacy_ema = BASE_MODEL_DIR / f"v3{self.revin_tag}_wm_best_ema.pt"
            legacy_latest = BASE_MODEL_DIR / f"v3{self.revin_tag}_wm_latest.pt"
            for p in (legacy_ema, legacy_latest):
                if p.exists():
                    model_path = p
                    print(f"  [INFO] using legacy ckpt {p.name}; consider renaming to {prefix}_wm_*")
                    break
        if not model_path.exists():
            print(f"  [ERROR] No checkpoint found in {BASE_MODEL_DIR} (looked for {prefix}_wm_*)")
            sys.exit(1)
        ...
        # 2026-05-03: LOUD-FAIL dim check BEFORE load. Catches silent
        # wrong-feature ckpt loads that would emit garbage IC/ShIC numbers.
        sd = (ckpt.get("model_state_dict") if isinstance(ckpt, dict) and "model_state_dict" in ckpt
              else ckpt.get("state_dict") if isinstance(ckpt, dict) and "state_dict" in ckpt
              else ckpt if isinstance(ckpt, dict) else None)
        if sd is not None:
            self._assert_dim_match(sd, model_path.name)
    ...
    def _assert_dim_match(self, sd: dict, ckpt_name: str) -> None:
        """LOUD-FAIL on input-dim mismatch before load_state_dict swallows it.

        Reads obs_encoder.0.weight from the state-dict (input feature dim) and
        compares to the expected n_features. A mismatch silently produces a
        wrong-shape model that emits garbage IC/ShIC numbers downstream;
        we'd rather fail with an informative error than ship bad numbers.
        """
        # Find the first encoder weight key with shape we can inspect.
        for k, v in sd.items():
            if k.startswith("obs_encoder") and k.endswith("weight") and v.ndim == 2:
                ckpt_input = int(v.shape[1])
                # Asset embedding is concatenated to input; subtract its dim if known.
                # (Falls through if we can't infer; conservative.)
                expected = self.n_features
                # First-cut: if matches exactly, OK.
                if ckpt_input == expected:
                    return
                # Account for asset embedding concat (e.g. V1.1 patterns)
                emb_dim = sd.get("asset_embedding.weight")
                if emb_dim is not None and emb_dim.ndim == 2:
                    if ckpt_input - emb_dim.shape[1] == expected:
                        return
                raise RuntimeError(
                    f"\n  [LOUD FAIL] DIM MISMATCH on ckpt {ckpt_name}\n"
                    f"  obs_encoder.0.weight has shape[1]={ckpt_input}, "
                    f"expected n_features={expected} (or n_features+asset_emb_dim).\n"
                    f"  This ckpt was trained with a DIFFERENT --features count. "
                    f"Pass --features N where N matches the training run, "
                    f"or use --model <full_path> with a ckpt that matches "
                    f"n_features={expected}.\n"
                    f"  Refusing to load -- silently loading wrong weights would "
                    f"emit garbage IC/ShIC numbers (per 'trust the numbers' bar)."
                )
        # No obs_encoder key found -> can't check; let load_state_dict catch it
        return
    ...
    def load_validation_data(self, use_full_data=False):
        ...
        for f in files:
            ...
            if not use_full_data:
                # FIX 2026-05-29: was 0.90 -> the "val" slice was the last 10%,
                # i.e. the UNSEEN held-out segment (contaminating it AND mis-measuring
                # IC). Val segment is [50%+purge, 70%], mirroring v1.0 (the correct one).
            ...
            feats, targets = extract_features_targets(
                df_val, FEATURE_LIST, REWARD_HORIZONS, asset_name
            )
    ...
    def _load_train_split(self):
        """Load training split (first 50%) for loss ratio computation."""
        files = sorted(DATA_DIR.glob("*_v51_chimera*.parquet"))
        segments = []
        for f in files:
            asset_name = f.stem.split("_")[0].upper()
            ...
            df = pl.read_parquet(f)
            df = selective_drop_nulls(df, FEATURE_LIST, REWARD_HORIZONS, asset_name)
            split = int(len(df) * 0.50)  # was 0.90: leaked OOS+unseen into train-loss baseline
            df_train = df.slice(0, split)
            feats, targets = extract_features_targets(
                df_train, FEATURE_LIST, REWARD_HORIZONS, asset_name
            )
            segments.append((feats, targets, asset_idx, asset_name))
        return segments
    ...
    def run_diagnostics(self):
        print(f"\n{'='*70}")
        print(f"  V3 WAVENET-GRU WORLD MODEL COMPREHENSIVE VALIDATION")
    ...
    def _evaluate_asset(self, feats, targets, asset_idx):
        seq_len = WM_SEQ_LEN
        results = {
            "reconstruction": {"total_mse": [], "per_feature_mse": [[] for _ in FEATURE_LIST],
                                "per_feature_r2": [[] for _ in FEATURE_LIST], "cosine_sim": []},
        ...
        for i in tqdm(indices, desc="  Evaluating", leave=False):
            ...
            cos_sim = F.cosine_similarity(recon.reshape(-1), obs.reshape(-1), dim=0).item()
            results["reconstruction"]["cosine_sim"].append(cos_sim)

            for fi in range(len(FEATURE_LIST)):
                feat_recon = recon[..., fi].reshape(-1).cpu().numpy()
                feat_real = obs[..., fi].reshape(-1).cpu().numpy()
            ...
            for mask_ratio in [0.0, 0.25, 0.50, 0.75]:
                ...
                imag_mse = F.mse_loss(imag_out["recon"], obs).item()
    ...
    def run_robust_validation(self, horizon=1):
        ...
        print(f"\n{'='*70}")
        print(f"  V3 WAVENET-GRU-RSSM ROBUST VALIDATION (Overfitting Detection)")
        print(f"  Model: {self.model_name}")
        ...
        output = {
            "version": "v3", "model": self.model_name,
            "timestamp": datetime.now().isoformat(),
...
def main():
    log_path = setup_logging(LOG_DIR, "v3_validate")
    parser = argparse.ArgumentParser(description="V3 WaveNet-GRU World Model Comprehensive Validation")
    ...
    parser.add_argument("--revin", action="store_true",
                        help="Enable RevIN (must match training config)")
    # 2026-05-03 cohort harmonization: feature-aware ckpt loading.
    # Without these flags, --latest/--both load legacy bare-name ckpts and
    # the WorldModelValidator silently uses settings.INPUT_DIM, which can
    # silently mismatch a ckpt trained with --features 29.
    parser.add_argument("--features", type=int, default=None,
                        help="Feature count of the trained ckpt (matches --features used at training). "
                             "Default: auto-discover from settings.INPUT_DIM. MUST match the ckpt's "
                             "training feature count or model load will silently load wrong-shape "
                             "weights and produce garbage IC/ShIC numbers.")
    parser.add_argument("--run-tag", type=str, default="",
                        help="Run-tag suffix used at training (e.g. 'baseline', 'sam', 'mtp'). "
                             "Threaded into ckpt path resolution; default empty = no suffix.")
    args = parser.parse_args()

    revin_tag = "_revin" if args.revin else ""
    n_features = args.features if args.features is not None else INPUT_DIM
    run_tag_str = f"_{args.run_tag}" if args.run_tag else ""
    # New ckpt prefix MUST mirror what the trainer wrote (see ddd5a39).
    prefix = f"v3_f{n_features}{revin_tag}{run_tag_str}"

    if args.both:
        print(f"\n  Running validation on BOTH checkpoints (prefix={prefix})...\n")
        candidates = [
            ("best_ema", BASE_MODEL_DIR / f"{prefix}_wm_best_ema.pt"),
            ("latest",   BASE_MODEL_DIR / f"{prefix}_wm_latest.pt"),
        ]
        # Legacy fallback: pre-rename bare names (only checked if new path missing).
        if not any(p.exists() for _, p in candidates):
            print(f"  [INFO] no ckpt at new prefix; falling back to legacy v3{revin_tag}_wm_*.pt")
            candidates = [
                ("best_ema", BASE_MODEL_DIR / f"v3{revin_tag}_wm_best_ema.pt"),
                ("latest",   BASE_MODEL_DIR / f"v3{revin_tag}_wm_latest.pt"),
            ]
        for name, path in candidates:
            if path.exists():
                print(f"\n{'#'*70}\n  CHECKPOINT: {name}  ({path.name})\n{'#'*70}")
                v = WorldModelValidator(path, use_revin=args.revin,
                                         n_features=n_features, run_tag=args.run_tag)
                if args.robust:
        ...
        if args.latest and not model_path:
            ...
            if not model_path.exists():
                # Legacy fallback
                legacy = BASE_MODEL_DIR / f"v3{revin_tag}_wm_latest.pt"
                if legacy.exists():
                    print(f"  [INFO] using legacy ckpt {legacy.name}")
                    model_path = legacy
        v = WorldModelValidator(model_path, use_revin=args.revin,
                                 n_features=n_features, run_tag=args.run_tag)
        passed = v.run_robust_validation(horizon=args.horizon)
        sys.exit(0 if passed else 1)
    elif args.model:
        v = WorldModelValidator(Path(args.model), use_revin=args.revin,
                                 n_features=n_features, run_tag=args.run_tag)
        passed = v.run_diagnostics()
        sys.exit(0 if passed else 1)
    elif args.latest:
        latest_path = BASE_MODEL_DIR / f"{prefix}_wm_latest.pt"
        if not latest_path.exists():
            legacy = BASE_MODEL_DIR / f"v3{revin_tag}_wm_latest.pt"
            if legacy.exists():
                print(f"  [INFO] using legacy ckpt {legacy.name}")
                latest_path = legacy
        v = WorldModelValidator(latest_path, use_revin=args.revin,
                                 n_features=n_features, run_tag=args.run_tag)
        passed = v.run_diagnostics()
        sys.exit(0 if passed else 1)
    else:
        v = WorldModelValidator(use_revin=args.revin,
                                 n_features=n_features, run_tag=args.run_tag)
        passed = v.run_diagnostics()
        sys.exit(0 if passed else 1)

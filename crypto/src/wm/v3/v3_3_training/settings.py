...
V3.3 Settings -- WaveNet-Only Variant (No GRU)



Ablation: Removes CausalGRU, feeds WaveNet TCN output directly to RSSM.

Tests whether the GRU's sequential state is redundant when WaveNet

already captures temporal context via dilated convolutions.
...
MODEL_DIR = PROJECT_ROOT / "models" / "wm" / "v3" / "v3_3"

LOG_DIR = PROJECT_ROOT / "logs" / "v3" / "v3_3"
...
TCN_DILATIONS = [1, 2, 4, 8]          # Exponential dilation for multi-scale
...
# V3.3: No GRU -- WaveNet output feeds directly to RSSM

USE_GRU = False  # default True in .1; False bypasses CausalGRU entirely
...
_TEMPORAL_DIM = TCN_CHANNELS[-1] if not USE_GRU else GRU_HIDDEN_DIM  # 256 either way

ADAPTER_FEAT_DIM = _TEMPORAL_DIM + FLAT_DIM  # 832 (cat(h_seq, z_post))
...
try:
    ...
    _assert_canonical(globals(), version_name="v3_3")
except ImportError:

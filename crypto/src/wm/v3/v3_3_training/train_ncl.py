"""
V3.3.D Diversity World Model Trainer -- Multi-Head NCL (Anti-Fragile Edition)

Architecture: V3.3 backbone (WaveNet-GRU-RSSM) + K parallel return heads with NCL
...
Key differences from train_world_model.py:
  ...
  - Can optionally load V3.3 backbone weights (--load-backbone)
  - Saves checkpoints as v3_1d_wm_*.pt
  - Uses same LR schedule, augmentation, and anti-fragile framework as V3

Anti-fragile training features (inherited from V3.3):
...
def train_diversity_model(load_backbone: str = None, freeze_backbone: bool = False,
                          n_features: int = 18):
    """
    Anti-fragile training loop for V3.3.D Diversity World Model.

    Same framework as V3 trainer but with:
    - DiversityWorldModel (K parallel return heads + NCL)
    - NCL penalty logging
    - Optional V3.3 backbone loading
    - v3_1d_* checkpoint naming

    Args:
        load_backbone: path to V3.3 checkpoint to load backbone from (optional)
    ...
    # -- Initialize Model ------------------------------------------------------
    print(f"\n  Initializing V3.3.D Diversity World Model...")
    model = DiversityWorldModel(input_dim=input_dim).to(DEVICE)

    # Optional: load backbone from V3.3.1 checkpoint
    if load_backbone:
        backbone_path = Path(load_backbone)
        if backbone_path.exists():
            print(f"  Loading V3.3 backbone from {backbone_path}")
    ...
    autopsy_path = NCL_MODEL_DIR / f"v3_3d_autopsy.jsonl"
    ...
    if not gate_passed:
        ...
        print("\n  [OK] V3.3.D Diversity model ready for downstream use.")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="V3.3.D Diversity World Model Trainer")
    parser.add_argument(
        "--load-backbone", type=str, default=None,
        help="Path to V3.3 checkpoint to initialize backbone weights from"
    )
    parser.add_argument(

"""
V6.1 World Model Validation -- Comprehensive Diagnostics Suite (Causal JEPA + Adversarial + Anti-Memorization)
...
V6.1-Specific Tests:
  ...
  - NO dream coherence (V6.1 has no dream_step)
...
class WorldModelValidator:
    """Comprehensive diagnostics for V6.1 Causal JEPA + Adversarial + Anti-Memorization World Model."""
    ...
    def __init__(self, model_path: Path = None, n_features: int = 22):
        ...
        self.ckpt_prefix = f"v6_1_{feat_tag}"
    ...
    def run_diagnostics(self):
        print(f"\n{'='*70}")
        print(f"  V6.1 CAUSAL JEPA + ADVERSARIAL + ANTI-MEMORIZATION COMPREHENSIVE VALIDATION")
    ...
    def _print_asset_report(self, r, name):
        ...
        # Representation Health (V6.1-specific)
        ...
        # Loss Components (V6.1-specific adversarial + anti-mem)
        lc = r.get("loss_components", {})
        if lc:
            print(f"\n  LOSS COMPONENTS (V6.1 Adversarial + Anti-Memorization)")
    ...
    def _save_results(self, all_results, gate_pass):
        output = {
            "version": "v6_1_causal_jepa_adversarial_antimem",
    ...
    def run_robust_validation(self, horizon=1):
        ...
        print(f"\n{'='*70}")
        print(f"  V6.1 CAUSAL JEPA + ADVERSARIAL + ANTI-MEM ROBUST VALIDATION (Overfitting Detection)")
        print(f"  Model: {self.model_name}")
        print(f"  Features: {self.n_features} (base_dim={self.base_dim})")
...
def main():
    log_path = setup_logging(LOG_DIR, "v6_1_validate")
    parser = argparse.ArgumentParser(
        description="V6.1 Causal JEPA + Adversarial + Anti-Memorization World Model Comprehensive Validation"
    )

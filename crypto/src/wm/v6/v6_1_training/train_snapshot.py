"""
V6.E Multi-Seed Ensemble Trainer -- K independent full training runs (Causal JEPA + Discriminator).
...
V6-specific (vs V2):
  - CausalJEPAWorldModel (CausalGRU + contrastive + VICReg + TimeDiscriminator)
  - get_loss() returns 4-tuple: (total_loss, loss_dict, l_disc, outputs) -- NOT 3-tuple!
  - DUAL optimizer: main (all except discriminator) + disc (discriminator only)
  - DUAL GradScaler: scaler_main + scaler_disc
  - Discriminator lr = WM_LR * DISC_LR_MULT, weight_decay=0.0
  - Training: main backward (retain_graph=True) -> main step -> disc backward -> disc step
  - model.update_target_encoder(JEPA_EMA_DECAY) every step (target encoder EMA)
  - update_ema(model, ema_model) every step (full-model EMA for stable val/ShIC)
  - No KL annealing, no Gumbel tau (JEPA has no stochastic latent)
  - model_type="jepa" for predict_fn
  - Checkpoint: save/load disc_optimizer + scaler_disc
"""
import torch
...
from train_world_model import collate_fn, validate, check_gate, get_mask_ratio, update_ema, set_lr
...
def save_master_checkpoint(seeds, completed, seed_metrics, feat_tag="f18"):
    ...
    state = {
        ...
        "version": "v6_1e_multi_seed_ensemble",
    }
    torch.save(state, ENSEMBLE_MODEL_DIR / f"v6_1e_{feat_tag}_master.pt")
...
def train_single_seed(
    ...
    """
    Train one full CausalJEPA model with a specific seed. Returns metrics dict.

    V6 requires:
      - DUAL optimizer (main + disc) and DUAL scaler (scaler_main + scaler_disc)
      - get_loss() returns 4-tuple (total, loss_dict, l_disc, outputs)
      - model.update_target_encoder(JEPA_EMA_DECAY) per step
      - NO KL annealing, NO Gumbel tau
    ...
    print(f"{'=' * 70}")
    print(f"  Architecture: Causal JEPA + Adversarial (d_model={WM_D_MODEL}, "
          f"d_latent={WM_D_LATENT}, n_layers={WM_N_LAYERS})")
    print(f"  Discriminator: hidden={DISC_HIDDEN}, lambda_adv={LAMBDA_ADV}, "
          f"lr_mult={DISC_LR_MULT}")
    ...
    # Main: all params EXCEPT discriminator, plus RevIN
    ...
    disc_optimizer = optim.AdamW(
        ...
        lr=WM_LR * DISC_LR_MULT,
        weight_decay=0.0,
        betas=(0.9, 0.95),
    )

    # -- DUAL GradScalers (V6-specific) ------------------------------------------
    ...
    print(f"  Main params: {main_n_params:,} | Disc params: {disc_n_params:,}")
    ...
    start_epoch = 0
    ckpt_path = ENSEMBLE_MODEL_DIR / f"v6_1e_{feat_tag}_seed_{seed_idx}_latest.pt"
    if ckpt_path.exists():
    ...
    autopsy_path = ENSEMBLE_MODEL_DIR / f"v6_1e_seed_{seed_idx}_autopsy.jsonl"
    ...
    # ==========================================================================
    # TRAINING LOOP (per seed) -- V6 JEPA+Adversarial
    # ==========================================================================
    for epoch in range(start_epoch, WM_TOTAL_EPOCHS):
        ...
        # V6 JEPA: NO KL annealing, NO Gumbel tau
        ...
        for step in pbar:
            ...
            # Forward + loss (V6: 4-tuple return from get_loss)
            ...
            # -- CRITICAL: Update target encoder (JEPA EMA) every step -----------
        ...
        # Kendall weights (V6: contrastive is at index 0, ret_1 at index 3)
        with torch.no_grad():
        ...
        print(
            ...
            f"adv:{avg_stats.get('adv', 0):.3f} disc:{avg_stats.get('disc', 0):.3f} | "
        ...
        if (epoch + 1) % WM_VAL_EVERY == 0 or epoch == WM_TOTAL_EPOCHS - 1:
            ...
            if (epoch + 1) % af_config.shuffled_ic_every == 0:
                ...
                if shuffled_ic > best_shuffled_ic:
                    ...
                    }, ENSEMBLE_MODEL_DIR / f"v6_1e_{feat_tag}_seed_{seed_idx}_best.pt")
            ...
            # Save per-seed checkpoint (includes disc_optimizer + dual scalers)
            state = {
                "epoch": epoch + 1,
                ...
                "seed": seed,
                "seed_idx": seed_idx,
                "version": "v6_1e_seed_training",
                "n_features": INPUT_DIM,
            }
            torch.save(state, ENSEMBLE_MODEL_DIR / f"v6_1e_{feat_tag}_seed_{seed_idx}_latest.pt")

            # Keep top-3 epoch checkpoints
            ep_path = ENSEMBLE_MODEL_DIR / f"v6_1e_{feat_tag}_seed_{seed_idx}_epoch_{epoch+1}.pt"
            ...
            print(
                ...
                f"adv:{val_metrics.get('adv', 0):.3f} disc:{val_metrics.get('disc', 0):.3f} | "
    ...
    final_path = ENSEMBLE_MODEL_DIR / f"v6_1_{feat_tag}_seed_{seed_idx}.pt"
    torch.save({
        "model_state_dict": ema_model.state_dict(),
        "revin_state_dict": revin.state_dict(),
    }, final_path)
    ...
    latest_ckpt = ENSEMBLE_MODEL_DIR / f"v6_1e_{feat_tag}_seed_{seed_idx}_latest.pt"
    if latest_ckpt.exists():
        try:
            latest_ckpt.unlink(missing_ok=True)
        except Exception:
            pass
    for ep_ckpt in ENSEMBLE_MODEL_DIR.glob(f"v6_1e_{feat_tag}_seed_{seed_idx}_epoch_*.pt"):
        try:
...
def train_ensemble(n_features: int = 18):
    """
    Multi-seed ensemble orchestrator for V6 Causal JEPA + Adversarial.
    Loads data once, then trains K independent models back-to-back with different seeds.
    """
    feature_list, input_dim, base_dim = get_feature_config(n_features)
    feat_tag = f"f{n_features}"

    log_path = setup_logging(LOG_DIR, f"v6_1e_{feat_tag}_ensemble_train")
    ...
    print("=" * 70)
    print("  V6.1.E MULTI-SEED ENSEMBLE TRAINER (Causal JEPA + Adversarial)")
    print("=" * 70)
    ...
    print_antifragile_header("V6.E", af_config)
    ...
    master_path = ENSEMBLE_MODEL_DIR / f"v6_1e_{feat_tag}_master.pt"
    ...
    for seed_idx in range(n_seeds):
        ...
        save_master_checkpoint(seeds, completed, seed_metrics, feat_tag=feat_tag)
    ...
    # -- Final Report --------------------------------------------------------------
    print("\n" + "=" * 70)
    print("  MULTI-SEED ENSEMBLE TRAINING COMPLETE (V6 Causal JEPA)")
    print("=" * 70)

    existing_seeds = sorted(ENSEMBLE_MODEL_DIR.glob(f"v6_1_{feat_tag}_seed_*.pt"))
...
if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description="V6.E Multi-Seed Ensemble Trainer")
    parser.add_argument("--features", type=int, choices=[13, 18], default=18,
                        help="Number of input features (14 or 19)")
    args = parser.parse_args()
    success = train_ensemble(n_features=args.features)
    if not success:

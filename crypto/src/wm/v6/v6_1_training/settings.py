...
V6.1 Settings -- Causal JEPA + Adversarial Time Shuffling + Anti-Memorization



Architecture: CausalGRU (NOT BiGRU) + JEPA + VICReg + Time Discriminator

Anti-Memorization: Feature-decoupled decoder (base features only), XD dropout/noise
...
MODEL_DIR = PROJECT_ROOT / "models" / "wm" / "v6" / "v6_1"

LOG_DIR = PROJECT_ROOT / "logs" / "v6" / "v6_1"
...
LAMBDA_ADV = 0.10         # Adversarial loss weight
...
try:
    ...
    _assert_canonical(globals(), version_name="v6_1")
except ImportError:

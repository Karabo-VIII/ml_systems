"""
V6.3.D Diversity World Model Trainer -- Multi-Head NCL (Anti-Fragile Edition)

Architecture: V6.3 backbone (CausalJEPA + VICReg + TimeDiscriminator) + K parallel return heads with NCL
...
Key differences from train_world_model.py:
  ...
  - Can optionally load V6.3 backbone weights (--load-backbone)
...
def train_diversity_model(load_backbone: str = None, freeze_backbone: bool = False,
    ...
    # -- Initialize Model ------------------------------------------------------
    print(f"\n  Initializing V6.3.D Diversity World Model...")
    model = DiversityWorldModel(input_dim=input_dim).to(DEVICE)

    # Optional: load backbone from V6.3.1 checkpoint
    if load_backbone:
        backbone_path = Path(load_backbone)
        if backbone_path.exists():
            print(f"  Loading V6.3 backbone from {backbone_path}")
    ...
    autopsy_path = NCL_MODEL_DIR / f"v6_3d_autopsy.jsonl"
    ...
    if not gate_passed:
        ...
        print("\n  [OK] V6.3.D Diversity model ready for downstream use.")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="V6.3.D Diversity World Model Trainer")
    parser.add_argument(
        "--load-backbone", type=str, default=None,
        help="Path to V6.3 checkpoint to initialize backbone weights from"
    )
    parser.add_argument(

...
def load_base_model(model_path=None):
    ...
    if model_path is None:
        model_path = BASE_MODEL_DIR / "v6_3_wm_best_ema.pt"
    if not model_path.exists():
        model_path = BASE_MODEL_DIR / "v6_3_wm_latest.pt"
    if not model_path.exists():
...
def load_adapter_model(base_model, adapter_path=None):
    ...
    if adapter_path is None:
        # Glob for adapter checkpoints (train saves as v6_3_{feat_tag}_adapter_*.pt)
        best_candidates = sorted(ADAPTER_MODEL_DIR.glob("v6_3_*_adapter_best.pt"))
        latest_candidates = sorted(ADAPTER_MODEL_DIR.glob("v6_3_*_adapter_latest.pt"))
        if best_candidates:

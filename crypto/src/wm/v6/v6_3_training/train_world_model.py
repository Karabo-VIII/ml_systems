"""
V6.3 World Model Trainer -- Causal JEPA + Adversarial + Anti-Memorization (Anti-Fragile Edition)

Architecture: Causal GRU JEPA + VICReg + Adversarial Time Discriminator + Feature Decoupling
Version: v6_3_causal_jepa_adversarial_antimem

Key V6.3 innovations (added vs V6):
  ...
  - Dynamic checkpoint prefix: v6_3_f13 or v6_3_f18
...
class CheckpointManager:
    ...
    V6.3-specific: also saves/loads discriminator optimizer state.
    Uses dynamic prefix (e.g. v6_3_f18, v6_3_f13) for feature ablation.
    """
...
def train_world_model(n_features: int = 18, use_revin: bool = False):
    """
    Anti-fragile training loop for V6.3 Causal JEPA + Adversarial + Anti-Memorization.
    ...
    ckpt_prefix = f"v6_3_{feat_tag}"
    log_path = setup_logging(LOG_DIR, f"v6_3_{feat_tag}_train")
    ...
    print("=" * 70)
    print("  V6.3 WORLD MODEL TRAINER (Causal JEPA + Adversarial + Anti-Memorization | Anti-Fragile)")
    print("=" * 70)
    ...
    print_antifragile_header("V6.3", af_config)
    ...
    for epoch in range(start_epoch, WM_TOTAL_EPOCHS):
        ...
        # V6.3: Adversarial weight schedule (ramp from start to end)
        _adv_start = globals().get("LAMBDA_ADV_START", LAMBDA_ADV)
        _adv_end = globals().get("LAMBDA_ADV_END", LAMBDA_ADV)
        _adv_anneal = globals().get("LAMBDA_ADV_ANNEAL_EPOCHS", 1)
        _adv_progress = min(1.0, epoch / max(1, _adv_anneal))
        model.current_lambda_adv = _adv_start + (_adv_end - _adv_start) * _adv_progress
        ...
        for step in pbar:
            ...
            # -- Main encoder backward (retain_graph for disc) -----------------
            optimizer.zero_grad(set_to_none=True)
...
if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description="V6.3 World Model Trainer (Anti-Memorization)")

...
V6.3 Settings -- JEPA Adversarial Weight Schedule Variant



Ablation: Ramps adversarial weight from 0 to 0.15 over 60 epochs

(vs fixed 0.10 in .1). Lets the encoder first learn good representations

via JEPA, then gradually introduces adversarial pressure.
...
MODEL_DIR = PROJECT_ROOT / "models" / "wm" / "v6" / "v6_3"

LOG_DIR = PROJECT_ROOT / "logs" / "v6" / "v6_3"
...
LAMBDA_ADV = 0.15  # V6.3: peak adversarial weight (was fixed 0.10)         # Adversarial loss weight

LAMBDA_ADV_START = 0.0    # V6.3: ramp from 0

LAMBDA_ADV_END = 0.15     # V6.3: ramp to 0.15

LAMBDA_ADV_ANNEAL_EPOCHS = 60  # V6.3: linear ramp over 60 epochs
...
try:
    ...
    _assert_canonical(globals(), version_name="v6_3")
except ImportError:

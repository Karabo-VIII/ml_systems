...
def run_validation(model_path=None):
    ...
    snapshot_ics = None
    master_path = ENSEMBLE_MODEL_DIR / "v6_3e_master.pt"
    ...
    if model_path:
        ...
        snapshot_paths = sorted(snap_dir.glob("v6_3e_seed_*_best.pt"))
        if not snapshot_paths:
            # Fallback to final models (backward compatibility)
            snapshot_paths = sorted(snap_dir.glob("v6_3_seed_*.pt"))
    else:

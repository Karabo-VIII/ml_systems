"""
V6.2 World Model -- Causal JEPA + Adversarial Time Shuffling + Anti-Memorization
...
Key Fix vs V6:
  V6.2 adds feature-decoupled anti-memorization:
...
class CausalJEPAWorldModel(nn.Module):
    """V6.2: Causal JEPA + Adversarial Time Shuffling + Anti-Memorization world model.
    ...
    def __init__(
        ...
        # ── Time Discriminator (V6.2: DISABLED -- ablation without adversarial)
        _use_disc = globals().get("USE_DISCRIMINATOR", True)
        self.use_discriminator = _use_disc
        if _use_disc:
    ...
    def get_loss(
        ...
        # ── 6. Adversarial loss (V6.2: DISABLED) ──────────────────────────────
        l_disc = torch.tensor(0.0, device=obs_seq.device)
        l_adv = torch.tensor(0.0, device=obs_seq.device)
        if self.use_discriminator:
            # Discriminator loss: distinguish real from time-shuffled
            real_score = self.discriminator(outputs["ctx_latent"].detach())
            perm = torch.randperm(T, device=obs_seq.device)
            shuffled_latent = outputs["ctx_latent"][:, perm, :].detach()
            fake_score = self.discriminator(shuffled_latent)
            ...
            encoder_score = self.discriminator(outputs["ctx_latent"])
...
if __name__ == "__main__":
    ...
    print(f"V6.2 Causal JEPA+Adversarial+AntiMem World Model Parameters: {total_params:,}")
    ...
    print("\nV6.2 Causal JEPA+Adversarial+AntiMem world model sanity check passed.")

...
Lightweight FiLM-based modulation layer that sits on top of a frozen V6 world model.
Provides regime-adaptive prediction corrections using:
  - Compressed V6 representations (ctx_latent bottleneck)
  - Context signals (rolling IC, prediction bias, regime, volatility)
  - Per-horizon scalar scale + shift (bounded, ~15K params)

Design principle: V6 provides deep structural knowledge (slow brain).
V6.x provides current regime adjustments (fast brain). V6.x can only
MODULATE predictions, never override them.

V6 SPECIAL CASE:
  - V6 does NOT have RSSM -- uses continuous latent space (ctx_latent, tgt_latent, pred_latent)
  - V6 get_loss returns 4 values: (total, loss_dict, l_disc, outputs) -- NOT 3
  - Adapter operates on ctx_latent [B, T, d_latent=192] only (no cat like V1)
  - Dual optimizer training (main + discriminator), but adapter only needs one optimizer

Classes:
  - AdaptiveResidualAdapter: Core FiLM modulation network
  - AdaptedWorldModel: Composite wrapper (frozen V6 + trainable adapter)
...
try:
    ...
    from v6_2_training.settings import (
        REWARD_HORIZONS, ADAPTER_FEAT_DIM, ADAPTER_CONTEXT_DIM,
        ADAPTER_BOTTLENECK, ADAPTER_FILM_HIDDEN,
        ADAPTER_MAX_SCALE_RANGE, ADAPTER_MAX_SHIFT_INIT,
        RETURN_HEAD_DIM, NUM_BINS,
...
class AdaptiveResidualAdapter(nn.Module):
    """
    FiLM-style modulation of V6's return trunk output.

    Takes V6's ctx_latent and a context vector,
    produces per-horizon scale and shift to modulate the return trunk.

    Architecture:
      feat [B,T,192] -> Linear(192, bottleneck) -> [B,T,16]
    ...
    def __init__(
        ...
        # Compress V6's 192-dim ctx_latent to bottleneck
    ...
    def forward(
        ...
        Args:
            feat:    [B, T, feat_dim] V6's ctx_latent
...
# =============================================================================
# ADAPTED WORLD MODEL (composite: frozen V6 + adapter)
# =============================================================================

class AdaptedWorldModel(nn.Module):
    """
    Composite model: frozen V6 base + trainable adapter.

    Drop-in replacement for CausalJEPAWorldModel in validation/inference.
    Runs V6 forward (no gradients), applies adapter modulation to the
    return trunk, then re-runs V6's frozen return heads on modulated trunk.

    V6-SPECIFIC: get_loss returns 4 values (total, loss_dict, l_disc, outputs)
    """
    ...
    def forward_train(
        ...
        # Get feat for adapter input (V6 uses ctx_latent only)
        feat = base_out["ctx_latent"]
        ...
        return {
            "ctx_latent": base_out["ctx_latent"],
            "tgt_latent": base_out["tgt_latent"],
            "pred_latent": base_out["pred_latent"],
    ...
    def encode_sequence(
        ...
        # V6 returns (ctx_latent, return_preds) for encode_sequence
    ...
    def get_loss(
        ...
        Uses the same loss function as V6 (bucketer.compute_loss) for strong
        ...
        V6-SPECIFIC: Returns 4 values to match V6's get_loss signature
        """
        outputs = self.forward_train(obs_seq, asset_id, context)
        ...
        # V6-SPECIFIC: return 4 values (total_loss, loss_dict, l_disc=0, outputs)
        # l_disc is always 0 for adapter (no discriminator in adapter)
        l_disc = torch.tensor(0.0, device=obs_seq.device)
        return total_loss, loss_dict, l_disc, outputs
...
class DriftMonitor:
    ...
    def set_baseline(self, ic_values: dict):
        """Set baseline IC from initial V6 validation."""
        for h in self.horizons:
            if h in ic_values:

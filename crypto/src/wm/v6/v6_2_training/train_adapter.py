...
Pipeline:
  1. Load frozen V6.2 base model (best checkpoint)
...
def main(n_features: int = 18):
    ...
    print("=" * 70)
    print("  V6.2.X ADAPTER TRAINER (FiLM Modulation | Rolling Window)")
    print("=" * 70)
    ...
    # -- Setup logging --------------------------------------------------------
    setup_logging(LOG_DIR, f"v6x_{feat_tag}_adapter_train")

    # -- Load frozen V6.2 base model -------------------------------------------
    base_ckpt_path = BASE_MODEL_DIR / f"v6_2_{feat_tag}_wm_best_ema.pt"
    if not base_ckpt_path.exists():
        base_ckpt_path = BASE_MODEL_DIR / f"v6_2_{feat_tag}_wm_best.pt"
    if not base_ckpt_path.exists():
        base_ckpt_path = BASE_MODEL_DIR / f"v6_2_{feat_tag}_wm_weights.pt"
    if not base_ckpt_path.exists():
        print(f"\n  [ERROR] No V6.2 checkpoint found in {BASE_MODEL_DIR}")
        print("  Train V6.2 first: python src/wm/v6/v6_2_training/train_world_model.py")
        teardown_logging()
        return

    print(f"\n  Loading base model from {base_ckpt_path.name}")
    base_model = CausalJEPAWorldModel(input_dim=input_dim, base_dim=base_dim).to(device_obj)
    ...
    if isinstance(state, dict) and "revin_state_dict" in state:
        ...
        print(f"  RevIN loaded from V6.2.1 checkpoint")
    else:
        print(f"  [WARN] No revin_state_dict in V6.2 checkpoint -- using freshly initialized RevIN")
    revin.eval()
    ...
    adapter_ckpt_path = ADAPTER_MODEL_DIR / f"v6_2_{feat_tag}_adapter_latest.pt"
    start_epoch = 0
    ...
    for epoch in range(start_epoch, ADAPTER_EPOCHS):
        ...
        if (epoch + 1) % ADAPTER_VAL_EVERY == 0 or epoch == ADAPTER_EPOCHS - 1:
            ...
            if val_results["ic_delta_mean"] > best_ic_delta:
                ...
                torch.save(best_state, ADAPTER_MODEL_DIR / f"v6_2_{feat_tag}_adapter_best.pt")
                save_marker = " *BEST*"
            ...
            torch.save(ckpt_state, ADAPTER_MODEL_DIR / f"v6_2_{feat_tag}_adapter_latest.pt")
    ...
    # -- Final summary --------------------------------------------------------
    print("\n" + "=" * 70)
    print("  V6.2.X ADAPTER TRAINING COMPLETE")
    print("=" * 70)
    print(f"  Best IC delta:       {best_ic_delta:+.4f}")
    print(f"  Adapter params:      {adapter_params:,}")
    print(f"  Base model hash:     {base_hash}")
    print(f"  Adapter checkpoint:  {ADAPTER_MODEL_DIR / f'v6_2_{feat_tag}_adapter_latest.pt'}")
    print(f"  Best adapter:        {ADAPTER_MODEL_DIR / f'v6_2_{feat_tag}_adapter_best.pt'}")
...
if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description="V6.x Adapter Trainer")

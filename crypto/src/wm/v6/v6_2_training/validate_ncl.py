...
Validates the V6.2.D DiversityWorldModel checkpoint from NCL_MODEL_DIR.
...
def load_model(model_path=None):
    """Load V6.2.D DiversityWorldModel + RevIN from NCL checkpoint."""
    model = DiversityWorldModel().to(DEVICE)
    revin = RevIN(num_features=INPUT_DIM).to(DEVICE)

    if model_path is None:
        ckpt_path = NCL_MODEL_DIR / "v6_1d_wm_best_ema.pt"
        if not ckpt_path.exists():
            ckpt_path = NCL_MODEL_DIR / "v6_1d_wm_latest.pt"
    else:

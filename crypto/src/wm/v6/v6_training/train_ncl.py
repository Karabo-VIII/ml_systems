"""
V6.D Diversity World Model Trainer -- Multi-Head NCL (Anti-Fragile Edition)

Architecture: V6 backbone (CausalJEPA + VICReg + TimeDiscriminator) + K parallel return heads with NCL
Version: v6d_diversity_ncl_antifragile
...
Key differences from train_world_model.py:
  ...
  - Can optionally load V6 backbone weights (--load-backbone)
  - Saves checkpoints as v6d_* (under NCL_MODEL_DIR from settings)
...
def train_diversity_model(load_backbone: str = None, freeze_backbone: bool = False, use_revin: bool = False, n_features: int = None):
    """
    Anti-fragile training loop for V6.D Diversity World Model.

    Same framework as V6 trainer but with:
    - DiversityWorldModel (K parallel return heads + NCL)
    - NCL penalty logging
    - Optional V6 backbone loading
    - v6d_* checkpoint naming
    - Dual optimizer (main + disc) matching V6's train_world_model.py

    Args:
        load_backbone: path to V6 checkpoint to load backbone from (optional)
        freeze_backbone: if True, freeze backbone and only train diversity heads
    """
    log_path = setup_logging(LOG_DIR, "v6d_train")
    ...
    # -- Initialize Model ------------------------------------------------------
    print(f"\n  Initializing V6.D Diversity World Model...")
    model = DiversityWorldModel(input_dim=input_dim).to(DEVICE)

    # Optional: load backbone from V6 checkpoint
    if load_backbone:
        backbone_path = Path(load_backbone)
        if backbone_path.exists():
            print(f"  Loading V6 backbone from {backbone_path}")
    ...
    autopsy_path = NCL_MODEL_DIR / f"v6d_autopsy.jsonl"
        ...
        if (epoch + 1) % WM_VAL_EVERY == 0 or epoch == DIVERSITY_TOTAL_EPOCHS - 1:
            ...
            if (epoch + 1) % af_config.shuffled_ic_every == 0:
                ...
                if shuffled_ic > best_shuffled_ic:
                    ...
                    torch.save(best_ema_state, NCL_MODEL_DIR / f"v6d{revin_tag}_wm_best_ema.pt")
            ...
            torch.save(state, NCL_MODEL_DIR / "v6d_wm_latest.pt")
            torch.save(model.state_dict(), NCL_MODEL_DIR / "v6d_wm_weights.pt")

            ep_path = NCL_MODEL_DIR / f"v6d_wm_epoch_{epoch+1}.pt"
    ...
    print(f"  EMA weights:         {NCL_MODEL_DIR / f'v6d{revin_tag}_wm_best_ema.pt'}")
    print(f"  Latest checkpoint:   {NCL_MODEL_DIR / 'v6d_wm_latest.pt'}")

    if not gate_passed:
        print("\n  [WARN] Diversity model did not pass validation gate.")
        print("  Do NOT proceed to agent training until gate criteria are met.")
    else:
        print("\n  [OK] V6.D Diversity model ready for downstream use.")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="V6.D Diversity World Model Trainer")
    parser.add_argument(
        "--load-backbone", type=str, default=None,
        help="Path to V6 checkpoint to initialize backbone weights from"
    )
    parser.add_argument(

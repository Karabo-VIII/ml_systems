...
def save_master_checkpoint(seeds, completed, seed_metrics):
    ...
    state = {
        ...
        "version": "v6e_multi_seed_ensemble",
    }
    torch.save(state, ENSEMBLE_MODEL_DIR / "v6e_master.pt")
...
def train_single_seed(
    ...
    # Main: all params EXCEPT discriminator, plus RevIN (if enabled)
    ...
    main_opt_params = list(main_params)
    if revin is not None:
        main_opt_params += list(revin.parameters())
    ...
    start_epoch = 0
    ckpt_path = ENSEMBLE_MODEL_DIR / f"v6e_seed_{seed_idx}_latest.pt"
    if ckpt_path.exists():
    ...
    autopsy_path = ENSEMBLE_MODEL_DIR / f"v6e_seed_{seed_idx}_autopsy.jsonl"
        ...
        if (epoch + 1) % WM_VAL_EVERY == 0 or epoch == WM_TOTAL_EPOCHS - 1:
            ...
            if (epoch + 1) % af_config.shuffled_ic_every == 0:
                ...
                if shuffled_ic > best_shuffled_ic:
                    ...
                    torch.save(best_ema_state, ENSEMBLE_MODEL_DIR / f"v6e_seed_{seed_idx}_best.pt")
            ...
            state = {
                ...
                "seed": seed,
                "seed_idx": seed_idx,
                "version": "v6e_seed_training",
                "n_features": n_features,
            }
            if revin is not None:
                state["revin_state_dict"] = revin.state_dict()
            torch.save(state, ENSEMBLE_MODEL_DIR / f"v6e_seed_{seed_idx}_latest.pt")

            # Keep top-3 epoch checkpoints
            ep_path = ENSEMBLE_MODEL_DIR / f"v6e_seed_{seed_idx}_epoch_{epoch+1}.pt"
    ...
    final_path = ENSEMBLE_MODEL_DIR / f"v6_seed_{seed_idx}.pt"
    ...
    latest_ckpt = ENSEMBLE_MODEL_DIR / f"v6e_seed_{seed_idx}_latest.pt"
    if latest_ckpt.exists():
        try:
            latest_ckpt.unlink(missing_ok=True)
        except Exception:
            pass
    for ep_ckpt in ENSEMBLE_MODEL_DIR.glob(f"v6e_seed_{seed_idx}_epoch_*.pt"):
        try:
...
def train_ensemble(use_revin: bool = False, n_features: int = None):
    ...
    """
    log_path = setup_logging(LOG_DIR, "v6e_ensemble_train")
    ...
    print("=" * 70)
    print("  V6.E MULTI-SEED ENSEMBLE TRAINER (Causal JEPA + Adversarial)")
    print("=" * 70)
    ...
    master_path = ENSEMBLE_MODEL_DIR / "v6e_master.pt"
    ...
    existing_seeds = sorted(ENSEMBLE_MODEL_DIR.glob("v6_seed_*.pt"))

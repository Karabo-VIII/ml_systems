"""
V6 World Model Validation -- Comprehensive Diagnostics Suite (Causal JEPA + Adversarial)
...
auxiliary reconstruction, regime classification, collapse detection, per-asset breakdown.
...
V6-Specific Tests:
  ...
  - NO dream coherence (V6 has no dream_step)
  - NO imagination masking (JEPA uses masking differently -- evaluated via reconstruction)
  - get_loss returns 3 values: (total_loss, loss_dict, l_disc)

Usage:
    python validate_world.py                    # Best model
...
class WorldModelValidator:
    """Comprehensive diagnostics for V6 Causal JEPA + Adversarial World Model."""
    ...
    def __init__(self, model_path: Path = None, use_revin: bool = False,
        ...
        self.model = CausalJEPAWorldModel().to(DEVICE)
    ...
    def _load_model(self, model_path: Path):
        ...
        prefix = f"v6_f{self.n_features}{self.revin_tag}{self.run_tag_str}"
        ...
        if not model_path.exists():
            ...
            for p in (BASE_MODEL_DIR / f"v6{self.revin_tag}_wm_best_ema.pt",
                       BASE_MODEL_DIR / "v6_wm_best.pt",
                       BASE_MODEL_DIR / f"v6{self.revin_tag}_wm_latest.pt"):
                if p.exists():
                    model_path = p
    ...
    def _assert_dim_match(self, sd: dict, ckpt_name: str) -> None:
        ...
        for k, v in sd.items():
            kl = k.lower()
            # V6 uses obs_proj rather than obs_encoder
            if (kl.startswith("obs_proj") or kl.startswith("obs_encoder")) and kl.endswith("weight") and v.ndim == 2:
    ...
    def load_validation_data(self, use_full_data=False):
        ...
        Args:
            use_full_data: If True, load all data (for robust validation).
                          If False, load the [50%+purge, 70%] val window (standard validation).
        """
    ...
    def run_diagnostics(self):
        print(f"\n{'='*70}")
        print(f"  V6 CAUSAL JEPA + ADVERSARIAL WORLD MODEL COMPREHENSIVE VALIDATION")
    ...
    def _evaluate_asset(self, feats, targets, asset_idx):
        ...
        results = {
            "reconstruction": {
                "total_mse": [],
                "per_feature_mse": [[] for _ in FEATURE_LIST],
                "per_feature_r2": [[] for _ in FEATURE_LIST],
            },
    ...
    def _print_asset_report(self, r, name):
        ...
        # Representation Health (V6-specific)
        print(f"\n  REPRESENTATION HEALTH (Causal JEPA + Adversarial)")
        ...
        # Loss Components (V6-specific adversarial)
        lc = r.get("loss_components", {})
        if lc:
            print(f"\n  LOSS COMPONENTS (V6 Adversarial)")
    ...
    def run_robust_validation(self, horizon=1):
        ...
        print(f"\n{'='*70}")
        print(f"  V6 CAUSAL JEPA + ADVERSARIAL ROBUST VALIDATION (Overfitting Detection)")
        print(f"  Model: {self.model_name}")
...
def main():
    log_path = setup_logging(LOG_DIR, "v6_validate")
    parser = argparse.ArgumentParser(
        description="V6 Causal JEPA + Adversarial World Model Comprehensive Validation"
    )
    ...
    prefix = f"v6_f{n_features}{revin_tag}{run_tag_str}"

    def _resolve(suffix: str) -> Path:
        new = BASE_MODEL_DIR / f"{prefix}_wm_{suffix}.pt"
        if new.exists():
            return new
        legacy = BASE_MODEL_DIR / f"v6{revin_tag}_wm_{suffix}.pt"
        if legacy.exists():

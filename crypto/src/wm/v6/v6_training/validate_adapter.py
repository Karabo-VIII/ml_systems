"""
V6.X Adapter Validation -- Compares adapted vs base model (CausalJEPA)
...
V6-SPECIFIC NOTES:
  - Base model: CausalJEPAWorldModel (NO RSSM, continuous latent space)
  - get_loss returns 4 values: (total, loss_dict, l_disc, outputs)
  - Output dict uses ctx_latent/tgt_latent/pred_latent (not z_post/h_seq)
  - V6 settings has no GATE_REC_MSE_MAX; uses a hardcoded fallback of 0.10
"""
import torch
...
PURGE_GAP_BARS = 400
GATE_REC_MSE_MAX = 0.10  # V6 settings has no GATE_REC_MSE_MAX; use fallback
...
def load_base_model(model_path=None, use_revin=True):
    """Load CausalJEPAWorldModel + RevIN from checkpoint.

    If use_revin=False, RevIN is not created and None is returned in its place.
    """
    if model_path is None:
        model_path = BASE_MODEL_DIR / "v6_wm_best_ema.pt"
    if not model_path.exists():
        model_path = BASE_MODEL_DIR / "v6_wm_latest.pt"
    if not model_path.exists():
    ...
    elif isinstance(ckpt, dict) and any(k.startswith("context_encoder") for k in ckpt):
        state_dict = ckpt
    else:
        state_dict = ckpt
...
def load_adapter_model(base_model, adapter_path=None):
    ...
    if adapter_path is None:
        for name in ["v6_adapter_best.pt", "v6_adapter_latest.pt"]:
            candidate = ADAPTER_MODEL_DIR / name
            if candidate.exists():
...
def compute_asset_ic(model, revin, feats, targets, asset_idx, use_revin=True):
    ...
    revin: RevIN instance or None. use_revin=True applies RevIN before forward.

    V6-SPECIFIC: output dict has ctx_latent/tgt_latent/pred_latent (no z_post).
    """
    seq_len = WM_SEQ_LEN
...
def run_validation(base_model_path=None, adapter_path=None, use_revin=True):
    print(f"\n{'='*65}")
    print(f"  V6.X ADAPTER VALIDATION (vs Base)")
    ...
    for feats, targets, asset_idx, asset_name in tqdm(segments, desc="  Base IC", leave=True):
        ics = compute_asset_ic(base_model, revin, feats, targets, asset_idx, use_revin=use_revin)
    ...
    for feats, targets, asset_idx, asset_name in tqdm(segments, desc="  Adapted IC", leave=True):
        ...
        ics = compute_asset_ic(adapted_model, revin, feats, targets, asset_idx, use_revin=use_revin)
    ...
    # V6 uses ctx_latent in recon output; recon key is present in forward_train
    rec_mses = []
    for feats, targets, asset_idx, asset_name in segments:
        obs_np = feats[:WM_SEQ_LEN]
        obs = torch.from_numpy(obs_np).unsqueeze(0).float().to(DEVICE)
        obs_norm = revin(obs, mode='norm') if revin is not None else obs
...
def main():
    parser = argparse.ArgumentParser(description="V6.X Adapter Validation vs Base")
    ...
    parser.add_argument("--revin", action="store_true", default=False,
                        help="Enable RevIN normalization (default: disabled)")

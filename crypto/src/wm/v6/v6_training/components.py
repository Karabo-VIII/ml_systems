...
class CausalGRUEncoder(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        Returns:
            h: [B, T, hidden_dim]
        """
        # CuDNN enabled for encoder GRU — only single backward flows through
        # here (loss.backward). The discriminator GRU needs CuDNN disabled
        # for WGAN-GP double backward, but the encoder does not.
        h, _ = self.gru(x)
        return self.norm(h)
...
class TimeDiscriminator(nn.Module):
    ...
    def forward(self, latent_seq: torch.Tensor) -> torch.Tensor:
        ...
        """
        # CuDNN enabled — WGAN-GP gradient penalty disabled (DISC_GRAD_PENALTY=0)
        # so no create_graph/double-backward needed. CuDNN GRU is 5-10x faster.

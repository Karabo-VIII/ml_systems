"""
V6.D NCL Diversity Validation -- Per-head IC + ensemble IC + diversity metrics

Validates the V6.D DiversityWorldModel checkpoint from NCL_MODEL_DIR.
V6 uses a CausalJEPA backbone (CausalGRU + contrastive + TimeDiscriminator), so there
are no KL/prior/posterior outputs -- focus is on IC and head diversity.

V6-specific vs V2.D:
  - Model is DiversityWorldModel from v6_training/ncl_model.py
  - Checkpoint prefix is v6d_* (not v2d_*)
  - forward_train returns ctx_latent/tgt_latent/pred_latent (same structure as V2)
  - TimeDiscriminator is present but not evaluated here (training-time only)
  - Reports adv/disc metrics from get_loss() for completeness
...
def load_model(model_path=None, use_revin: bool = False):
    """Load V6.D DiversityWorldModel + RevIN from NCL checkpoint."""
    ...
    if model_path is None:
        ckpt_path = NCL_MODEL_DIR / "v6d_wm_best_ema.pt"
        if not ckpt_path.exists():
            ckpt_path = NCL_MODEL_DIR / "v6d_wm_latest.pt"
    else:
    ...
    print(f"  [OK] V6.D loaded  |  heads={K}  |  NCL lambda={model.ncl_lambda}")
...
def evaluate(model, revin, segments):
    ...
    V6-specific: forward_train returns ctx_latent/tgt_latent/pred_latent instead
    of RSSM prior/post logits. TimeDiscriminator is not evaluated here.
    ...
    # Accumulators: ensemble and per-head predictions
    ...
    for feats, targets, asset_idx, asset_name in tqdm(segments, desc="  Assets", leave=True):
        ...
        for i in indices:
            ...
            # Ensemble logits (averaged across K heads)
            ...
            # Per-head logits from all_return_logits
...
def compute_diversity_metrics(head_preds, ensemble_reals, K):
    ...
    head_preds[k][1] = list of np.arrays from each sequence window.
    Returns:
        corr_matrix: [K, K] Pearson correlation between head predictions
        avg_off_diag: average off-diagonal correlation (lower = more diverse)
        mean_head_std: mean std across K heads per timestep (higher = more diverse)
    """
    preds_by_head = []
    for k in range(K):
    ...
    # Clip to same length (guard for edge cases)
    ...
    # Per-timestep std across K heads
...
def check_gates(mean_ic, shuffled_ic, ic_h1=None):
    ...
    if abs(mean_ic) > 1e-6:
        ...
        gates["Shuffled IC Ratio"] = (shic_pass, f"{ratio:.3f} > {GATE_SHUFFLED_IC_RATIO_MIN}")
    else:
...
def print_report(asset_results, ensemble_preds, ensemble_reals, head_preds,
    ...
    print(f"\n{'='*70}")
    print(f"  V6.D NCL DIVERSITY VALIDATION")
    print(f"  Model: {model_name}")
    print(f"  Time:  {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    print(f"  Heads: {K}  |  Horizons: {REWARD_HORIZONS}")
    print(f"  Arch: CausalJEPA (CausalGRU + contrastive + TimeDiscriminator)")
    print(f"{'='*70}")
    ...
    # Per-head IC across all horizons
    ...
    # Gates
    ic_h1_val = global_ics.get(1, mean_ic)
    all_pass, gates = check_gates(mean_ic, shuffled_ic, ic_h1=ic_h1_val)
    print(f"\n{'='*70}")
    ...
    if all_pass:
        ...
        print(f"  V6.D NCL model is validated.")
    else:
        print(f"  VERDICT: GATE(S) FAILED")
        print(f"  V6.D NCL model needs more training or tuning.")
    print(f"  {'='*40}")
...
def main():
    parser = argparse.ArgumentParser(description="V6.D NCL Diversity Validation")
    parser.add_argument("--model", type=str, default=None,
                        help="Path to specific NCL checkpoint")
    parser.add_argument("--revin", action="store_true",
                        help="Enable RevIN normalization")
    args = parser.parse_args()

    print(f"\n{'='*70}")
    print(f"  V6.D NCL DIVERSITY VALIDATOR")
    print(f"  NCL_MODEL_DIR: {NCL_MODEL_DIR}")
    print(f"  DEVICE: {DEVICE}")
    print(f"  RevIN: {'on' if args.revin else 'off'}")
    print(f"{'='*70}\n")

    # Load model
    model, revin, model_name = load_model(args.model, use_revin=args.revin)
    K = model.n_diversity_heads
    ...
    # Global ensemble IC (for gate + JSON)
    global_ics = {}
    for h in REWARD_HORIZONS:
        p = np.concatenate(ensemble_preds[h])
        r = np.concatenate(ensemble_reals[h])

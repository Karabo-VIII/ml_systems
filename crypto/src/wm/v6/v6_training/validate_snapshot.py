"""
V6.E Snapshot Ensemble Validation
...
def compute_single_snapshot_ic1(snap_model, revin, feats, targets, asset_idx):
    ...
    Applies RevIN manually if provided (using model._revins[i] pattern).
    """
    seq_len = WM_SEQ_LEN
    asset = torch.tensor([asset_idx], dtype=torch.long, device=DEVICE)
    indices = list(range(0, len(feats) - seq_len, seq_len))
...
def compute_global_shuffled_ic(model, segments, n_seeds=5, batch_size=32):
    ...
    SnapshotEnsemble handles RevIN internally -- no external RevIN applied.
    V6 JEPA: no temporal continuity in features, so shuffled IC tests whether
    the model genuinely learned cross-sectional signal vs temporal patterns.
    """
    all_ics = []
...
def check_gates(avg_rec_mse, mean_ic, shuffled_ic, ic_h1=None):
    ...
    # V6 JEPA gates: no KL divergence (contrastive, not VAE-style)
    # V6 settings.py has no GATE_REC_MSE_MAX constant; use 0.10 as threshold
    GATE_REC_MSE_THRESHOLD = 0.10

    gates = [
        ("Reconstruction MSE",      avg_rec_mse < GATE_REC_MSE_THRESHOLD,
         f"{avg_rec_mse:.5f} < {GATE_REC_MSE_THRESHOLD}"),
...
def run_validation(model_path=None, use_revin: bool = False):
    print(f"\n{'='*65}")
    print(f"  V6.E SNAPSHOT ENSEMBLE VALIDATION")
    print(f"  Time: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    print(f"  Device: {DEVICE}")
    print(f"  RevIN: {'on' if use_revin else 'off'} (per-snapshot, via SnapshotEnsemble)")
    print(f"{'='*65}")
    ...
    if master_path.exists():
        try:
            ...
            if isinstance(master, dict) and "seed_metrics" in master:
                ...
                print(f"  [OK] Loaded seed ICs from v6e_master.pt")
        except Exception:
            pass
    ...
    if model_path:
        ...
        snapshot_paths = sorted(snap_dir.glob("v6e_seed_*_best.pt"))
        if not snapshot_paths:
            # Fallback to final models (backward compatibility)
            snapshot_paths = sorted(snap_dir.glob("v6_seed_*.pt"))
    else:
...
def main():
    parser = argparse.ArgumentParser(description="V6.E Snapshot Ensemble Validation")
    parser.add_argument("--model", type=str, default=None,
                        help="Path to directory containing snapshot checkpoints")
    parser.add_argument("--revin", action="store_true",
                        help="Enable RevIN normalization (per-snapshot, via SnapshotEnsemble)")
    args = parser.parse_args()

    passed = run_validation(model_path=args.model, use_revin=args.revin)

"""
V1.x Adapter Trainer -- Rolling Window Adaptation

Trains a lightweight FiLM adapter on top of a frozen V1 world model.
The adapter learns regime-specific corrections to V1's return predictions.

Pipeline:
  1. Load frozen V1 base model (best EMA checkpoint)
  2. Load data, split train/val via walk-forward
  3. Build context vector from TRAINING data (not val -- prevents leakage)
  4. Train adapter on-the-fly: each step runs frozen V1 + adapter forward
  5. Validate: adapted IC vs base IC per horizon
  6. Save adapter if it improves over base

On-the-fly design (no pre-caching):
  - Each training step runs frozen V1 forward (no_grad) then adapter forward
  - Avoids 40GB+ memory for cached representations
  - V1 forward under no_grad is fast (~20ms per batch on RTX 4060)
...
def compute_context_from_segments(
    base_model: TransformerWorldModel,
    segments: list,
    device: torch.device,
    lookback: int = ADAPTER_CONTEXT_LOOKBACK,
) -> np.ndarray:
    """
    Compute static context vector (12 of 17 dims) from the tail of given segments.
    ...
    for seg in segments:
        ...
        for i in range(0, len(feats_window) - WM_SEQ_LEN, WM_SEQ_LEN):
            ...
            base_obs = obs[:, :, :INPUT_DIM]  # [1, T, 13] for V1 base model
            asset = torch.tensor([asset_idx], dtype=torch.long, device=device)

            with torch.no_grad():
                outputs = base_model.forward_train(base_obs, asset)

            preds, acts = {}, {}
            for h in REWARD_HORIZONS:
                pred = base_model.bucketer.decode(
                    outputs["return_logits"][h]
                ).cpu().numpy().flatten()
...
def validate_adapter(
    ...
    for seg in val_segments:
        ...
        for batch_start in range(0, len(indices), 64):
            ...
            # Split: base features for frozen V1, XD for context
            base_obs = obs[:, :, :INPUT_DIM]   # [B, T, 13]
            xd_last = obs[:, -1, INPUT_DIM:]   # [B, 5]
            static_ctx = context[:12].unsqueeze(0).expand(len(obs_list), -1).to(device)
            ctx = torch.cat([static_ctx, xd_last], dim=-1)  # [B, 17]

            # Use adapted_model which returns both adapted and base logits
            adapted_out = adapted_model.forward_train(base_obs, asset, ctx)
...
def main():
    print("=" * 70)
    print("  V1.x ADAPTER TRAINER (FiLM Modulation | Rolling Window)")
    print("=" * 70)
    print(f"  Device:         {DEVICE}")
    print(f"  Base model:     TransformerWorldModel (frozen)")
    ...
    # -- Setup logging --------------------------------------------------------
    setup_logging(LOG_DIR, "v1x_adapter_train")

    # -- Load frozen V1 base model -------------------------------------------
    base_ckpt_path = BASE_MODEL_DIR / "v1_0_f13_wm_best_ema.pt"
    if not base_ckpt_path.exists():
        base_ckpt_path = BASE_MODEL_DIR / "v1_0_f13_wm_weights.pt"
    if not base_ckpt_path.exists():
        print(f"\n  [ERROR] No V1.0 checkpoint found in {BASE_MODEL_DIR}")
        print("  Train V1.0 first: python src/wm/v1/v1_0_training/train_world_model.py")
        teardown_logging()
        return

    print(f"\n  Loading base model from {base_ckpt_path.name}")
    base_model = TransformerWorldModel().to(device_obj)
    ...
    if isinstance(state, dict) and "ema_state_dict" in state:
        base_model.load_state_dict(state["ema_state_dict"])
    ...
    # Load 18 features: 13 base for frozen V1, 5 XD for adapter context
    ...
    adapter_ckpt_path = MODEL_DIR / "v1_adapter_latest.pt"
    start_epoch = 0
    ...
    for epoch in range(start_epoch, ADAPTER_EPOCHS):
        ...
        for step in pbar:
            ...
            # Split: base features (13) for frozen V1, XD features (5) for context
            base_obs = obs_batch[:, :, :INPUT_DIM]   # [B, T, 13]
            xd_last = obs_batch[:, -1, INPUT_DIM:]   # [B, 5] last bar's XD values

            # Context: 12 static (with noise) + 5 XD (per-sample, from last bar)
            ctx_noise = torch.randn(12, device=device_obj) * 0.1
            static_ctx = (context_tensor[:12] + ctx_noise).unsqueeze(0).expand(
                base_obs.size(0), -1
            )  # [B, 12]
            ctx_batch = torch.cat([static_ctx, xd_last], dim=-1)  # [B, 17]

            optimizer.zero_grad(set_to_none=True)

            # Run frozen V1 forward on base features only (never sees XD)
            with torch.no_grad():
                base_out = base_model.forward_train(base_obs, asset_batch)

            # Get V1's combined representation and trunk
            feat = torch.cat([base_out["h_seq"], base_out["z_post"]], dim=-1).detach()
        ...
        if (epoch + 1) % ADAPTER_VAL_EVERY == 0 or epoch == ADAPTER_EPOCHS - 1:
            ...
            if val_results["ic_delta_mean"] > best_ic_delta:
                ...
                best_state = {
                    ...
                    "version": "v1x_film_adapter_v2",
                }
                torch.save(best_state, MODEL_DIR / "v1_adapter_best.pt")
                save_marker = " *BEST*"
            ...
            torch.save(ckpt_state, MODEL_DIR / "v1_adapter_latest.pt")
    ...
    # -- Final summary --------------------------------------------------------
    print("\n" + "=" * 70)
    print("  V1.x ADAPTER TRAINING COMPLETE")
    print("=" * 70)
    print(f"  Best IC delta:       {best_ic_delta:+.4f}")
    print(f"  Adapter params:      {adapter_params:,}")
    print(f"  Base model hash:     {base_hash}")
    print(f"  Adapter checkpoint:  {MODEL_DIR / 'v1_adapter_latest.pt'}")
    print(f"  Best adapter:        {MODEL_DIR / 'v1_adapter_best.pt'}")

"""
V1 World Model Validation — Comprehensive Diagnostics Suite
...
class WorldModelValidator:
    """Comprehensive diagnostics for V1 Transformer-RSSM World Model."""

    def __init__(self, model_path: Path = None):
        self.model = TransformerWorldModel().to(DEVICE)
        self.model_name = "unknown"
        self.revin = None  # V1.0 has no RevIN
        self._load_model(model_path)
        self.model.eval()

    def _load_model(self, model_path: Path):
        if model_path is None:
            model_path = BASE_MODEL_DIR / "v1_0_f13_wm_best_ema.pt"
        if not model_path.exists():
            model_path = BASE_MODEL_DIR / "v1_0_f13_wm_latest.pt"
        if not model_path.exists():
    ...
    def load_validation_data(self, use_full_data=False):
        ...
        for f in files:
            ...
            if not use_full_data:
                # Use val segment (60-80%) matching training split, NOT last 10%
                # Previous: split = int(len(df) * 0.90) evaluated on unseen, not val
                val_start = int(len(df) * 0.50) + self.PURGE_GAP_BARS
                val_end = int(len(df) * 0.70)
                if val_start >= val_end:
                    val_start = int(len(df) * 0.50)  # fallback
                    val_end = int(len(df) * 0.70)
    ...
    def run_diagnostics(self):
        print(f"\n{'='*70}")
        print(f"  V1 WORLD MODEL COMPREHENSIVE VALIDATION")
            ...
            for h in REWARD_HORIZONS:
                ...
                for feats, targets, asset_idx, asset_name, *_extra in segments:
                    ...
                    if n_pred_bars != len(voladj_preds):
                        # Mismatch from masking -- fall back to min length
    ...
    def _load_train_split(self):
        ...
        for f in files:
            ...
            split = int(len(df) * 0.50)  # Train segment only (50%), not 90%
    ...
    def run_robust_validation(self, horizon=1):
        ...
        print(f"\n{'='*70}")
        print(f"  V1 TRANSFORMER-RSSM ROBUST VALIDATION (Overfitting Detection)")
        print(f"  Model: {self.model_name}")
        ...
        output = {
            "version": "v1", "model": self.model_name,
            "timestamp": datetime.now().isoformat(),
...
def main():
    log_path = setup_logging(LOG_DIR, "v1_validate")
    parser = argparse.ArgumentParser(description="V1 World Model Comprehensive Validation")
    ...
    if args.both:
        ...
        for name, path in [("best_ema", BASE_MODEL_DIR / "v1_0_f13_wm_best_ema.pt"),
                           ("latest", BASE_MODEL_DIR / "v1_0_f13_wm_latest.pt")]:
            if path.exists():
                print(f"\n{'#'*70}")
                print(f"  CHECKPOINT: {name}")
                print(f"{'#'*70}")
                v = WorldModelValidator(path)
                if args.robust:
        ...
        if args.latest and not model_path:
            model_path = BASE_MODEL_DIR / "v1_0_f13_wm_latest.pt"
        v = WorldModelValidator(model_path)
        passed = v.run_robust_validation(horizon=args.horizon)
        sys.exit(0 if passed else 1)
    elif args.model:
        v = WorldModelValidator(Path(args.model))
        passed = v.run_diagnostics()
        sys.exit(0 if passed else 1)
    elif args.latest:
        v = WorldModelValidator(BASE_MODEL_DIR / "v1_0_f13_wm_latest.pt")
        passed = v.run_diagnostics()
        sys.exit(0 if passed else 1)
    else:
        v = WorldModelValidator()
        passed = v.run_diagnostics()
        sys.exit(0 if passed else 1)

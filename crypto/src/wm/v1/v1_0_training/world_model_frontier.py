...
class TransformerWorldModel(nn.Module):
    ...
    def _get_stoch_state(self, logits: torch.Tensor) -> torch.Tensor:
        ...
        Args:
            logits: [..., flat_dim] where flat_dim = latent_dim * classes

        Returns:
            [..., flat_dim] one-hot samples (hard in forward, soft in backward)
        """
        shape = logits.shape
    ...
    def get_loss(
        ...
        # -- KL Divergence (torch.distributions for AMP float16 stability) ----
        ...
        # D.kl_divergence handles 0*-inf edge cases that manual computation misses
        ...
        # All ACTIVE_HORIZONS contribute to loss (h16/h64 = multi-scale regularizers)
        ...
        # Without this, regime drifts back to ~1.0x weight and collapses to neutral
        ...
        # Add direct return loss (fixed weight, not uncertainty-weighted)
        total = total + DIRECT_RETURN_WEIGHT * l_direct_return

        # Add pairwise ranking loss (fixed weight)
        if PAIRWISE_RANK_WEIGHT > 0:
            total = total + PAIRWISE_RANK_WEIGHT * l_pairwise
        ...
        if hasattr(self, 'add_frontier_losses') and self.training:
            total, loss_dict = FrontierLossMixin.add_frontier_losses(
                self, total, loss_dict, outputs, target_returns,

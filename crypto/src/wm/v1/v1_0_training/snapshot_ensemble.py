...
class EnsembleGating(nn.Module):
    """
    XD-conditioned soft weighting of K snapshot models.

    Linear(xd_dim, K) -> softmax -> per-model weights [B, K].
    Initialized near-uniform (zeros) so untrained gating = uniform averaging.
    ~5*K parameters -- structurally cannot overfit.
    """

    def __init__(self, xd_dim: int = XD_DIM, n_models: int = None):
        super().__init__()
        self.xd_dim = xd_dim
        self.n_models = n_models
        self._gate = None
        if n_models is not None:
            self._build_gate(n_models)

    def _build_gate(self, n_models: int):
        self.n_models = n_models
        self._gate = nn.Linear(self.xd_dim, n_models, bias=True)
        nn.init.zeros_(self._gate.weight)
        nn.init.zeros_(self._gate.bias)

    def forward(self, xd_features: torch.Tensor) -> torch.Tensor:
        """
        Args:
            xd_features: [B, 5] XD values from last bar of sequence
        Returns:
            weights: [B, K] normalized gating weights (sum=1 per sample)
        """
        if self._gate is None:
            raise RuntimeError("Call _build_gate(n_models) first")
        return F.softmax(self._gate(xd_features), dim=-1)
...
class SnapshotEnsemble(nn.Module):
    ...
    def __init__(self, snapshot_paths: list = None, top_k: int = SNAPSHOT_TOP_K,
        ...
        if snapshot_paths is None:
            ...
            snapshot_paths = sorted(snap_dir.glob("v1_snapshot_*.pt"))
    ...
    def forward_train(self, obs_seq, asset_id, masked_obs_seq=None, xd_features=None):
        ...
        # Weighted or uniform average of return logits
        avg_return_logits = {}
        if xd_features is not None and hasattr(self, 'gating') and self.gating is not None:
            weights = self.gating(xd_features)  # [B, K]
            for h in REWARD_HORIZONS:
                # [K, B, T, NUM_BINS] -> [B, K, T, NUM_BINS]
                stacked = torch.stack(
    ...
    def get_loss(self, obs_seq, asset_id, target_returns, mask_ratio=0.0, block_mask=False):
        ...
        # Reconstruction (decoder outputs INPUT_DIM=13 dims, match obs_seq[:,:,:INPUT_DIM])
        l_rec = F.mse_loss(outputs["recon"], obs_seq[:, :, :INPUT_DIM])

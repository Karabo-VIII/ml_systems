...
__contract__ = {
    ...
    "model": "V1.1",
    "outputs": {
        "checkpoints": "models/wm/v1/v1_1/v1_1_f<N>_wm_*.pt",
    },
    "invariants": {
        "WM_BATCH_SIZE":              32,
        "BIN_MIN":                   -1.0,
        ...
        "xd_anti_memorization":       "base_dim/input_dim split for f34+",
    },
    "rationale": "Record holder: f34 IC=0.0674 ShIC=0.0330 (V family best).",
}

"""
V1.1 Settings -- Transformer-RSSM World Model (41-Feature Flexible)

Based on V1/TMP_2 proven architecture (ShIC=0.0305, GATE PASS, 105 epochs).
41 features (34 base + 7 cross-asset), selectable via --features 13/18/21/25/30/34/37/41.

Architecture: Causal Transformer + RSSM Latents
Hardware Target: RTX 4060 (8GB) + i9, Windows 11
Data: 10 assets, 41 features (34 base + 7 cross-asset)

Feature groups (prefix-slice subsets of the master FEATURE_LIST):
  - f13:  Core per-asset features (deviation, fd_close, vpin, flow, etc.)
  - f18:  Extended (+return_4, return_16, kurtosis, bar_duration, funding_momentum)
  - f21:  f18 + Hawkes intensity features
  - f25:  f21 + momentum_accel, vol_price_corr, vol_ratio, flow_persistence
  - f30:  f25 + SOTA microstructure (yz_vol, cs_spread, perm_entropy, kyle_lambda, oi_price_div)
  - f34:  f30 + 4 XD cross-asset features (BEST: IC=0.0674, ShIC=0.0330, all-time record)
  - f37:  f34 + xd_funding_spread, xd_cross_return_mean, xd_cross_vol_mean
  - f41:  Full 41-feature set
...
SOTA components:
  ...
  - Multi-head feature ablation (--ablation) for marginal contribution analysis
...
DATA_DIR = PROJECT_ROOT / "data" / "processed" / "chimera" / "dollar"  # v51 (was chimera_legacy/dollar=v50; migrated 2026-05-17 per connector_integrity_crawler A1)
MODEL_DIR = PROJECT_ROOT / "models" / "wm" / "v1" / "v1_1"
LOG_DIR = PROJECT_ROOT / "logs" / "v1" / "v1_1"
...
CROSS_ENSEMBLE_DIR = MODEL_DIR / "cross_ensemble"
for _d in [MODEL_DIR, LOG_DIR, BASE_MODEL_DIR, ADAPTER_MODEL_DIR, ENSEMBLE_MODEL_DIR, NCL_MODEL_DIR, CROSS_ENSEMBLE_DIR]:
...
# V1.1 supports the full registry (V1.x is the lead feature-test version)
SUPPORTED_FEATURE_COUNTS_V1_1 = (13, 18, 21, 25, 29, 30, 34, 37, 41,
...
def get_feature_config(n_features: int):
    ...
    For all v51 frontier cuts (f46+), base_dim=41 (the v50 41 features are
    the memorization-anchored base; the frontier 5-80 features are XD-style
    supplementary inputs subject to dropout/noise regularization).
    """
    if n_features not in SUPPORTED_FEATURE_COUNTS_V1_1:
        raise ValueError(
            f"V1.1 supports {sorted(SUPPORTED_FEATURE_COUNTS_V1_1)}; got f{n_features}"
        )
...
def get_ablation_subsets(n_features: int) -> dict:
    ...
    """
    if n_features <= 13:
        return {}  # Nothing smaller

    if n_features == 17:
        return {"f13": list(range(13))}

    if n_features == 18:
        # f18 = base13 + XD5 — only f13 valid (zeroes XD at [13:18])
        return {"f13": list(range(13))}

    if n_features == 20:
        # f20 = base13 + ext4 + tier1_3 (contiguous)
        return {
            "f13": list(range(13)),
            "f17": list(range(17)),
        }
    ...
    if n_features == 22:
        # f22 = base13 + ext4 + XD5 — f20 INVALID (no Tier 1 features)
        return {
            "f13": list(range(13)),
            "f17": list(range(17)),  # zeroes XD at [17:22]
        }

    if n_features == 25:
        # f25 = base13 + ext4 + tier1_3 + XD5 (contiguous)
        return {
            "f13": list(range(13)),
            "f17": list(range(17)),
            "f20": list(range(20)),  # zeroes XD at [20:25]
        }
...
if HEADLINE_MODE:
    # CC-H4 anti-mem stack: tighten the three available regularizers
    WM_FREE_NATS = 1.5             # was 1.0; raise KL-throw-away floor (V1.x knob 1)
    XD_DROPOUT_RATE = 0.85         # was 0.7;  drop ~12 of 34 per batch (V1.x knob 2)
    XD_NOISE_STD = 0.4             # was 0.3;  bump noise on dropped features (V1.x knob 3, 2026-05-10)
    # Pattern P+Q at f29: REC_LOG_VAR_CLAMP_MIN already 0.5 (Pattern Q applied invariantly)
    # Pattern P (f29) chosen at CLI via --features 29
    print(f"[V1.1 HEADLINE_MODE] WM_FREE_NATS=1.5 XD_DROPOUT_RATE=0.85 XD_NOISE_STD=0.4 -- "
          f"target ShIC>=0.045, ratio>=0.65 per WM_HEADLINE_UPGRADE_PLAN §0 CC-H4")
...
try:
    ...
    _assert_canonical(globals(), version_name="v1_1")
except ImportError:
    pass

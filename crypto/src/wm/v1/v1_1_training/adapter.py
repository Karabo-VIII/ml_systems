...
Lightweight FiLM-based modulation layer that sits on top of a frozen V1 world model.
Provides regime-adaptive prediction corrections using:
  - Compressed V1 representations (feat bottleneck)
  - Context signals (rolling IC, prediction bias, regime, volatility)
  - Per-horizon scalar scale + shift (bounded, ~15K params)

Design principle: V1 provides deep structural knowledge (slow brain).
V1.x provides current regime adjustments (fast brain). V1.x can only
MODULATE predictions, never override them.

Classes:
  - AdaptiveResidualAdapter: Core FiLM modulation network
  - AdaptedWorldModel: Composite wrapper (frozen V1 + trainable adapter)
...
class AdaptiveResidualAdapter(nn.Module):
    """
    FiLM-style modulation of V1's return trunk output.

    Takes V1's combined representation (feat) and a context vector,
    ...
    def __init__(
        ...
        # Compress V1's 832-dim representation to bottleneck
    ...
    def forward(
        ...
        Args:
            feat:    [B, T, feat_dim] V1's cat(h_seq, z_post)
...
# =============================================================================
# ADAPTED WORLD MODEL (composite: frozen V1 + adapter)
# =============================================================================

class AdaptedWorldModel(nn.Module):
    """
    Composite model: frozen V1 base + trainable adapter.

    Drop-in replacement for TransformerWorldModel in validation/inference.
    Runs V1 forward (no gradients), applies adapter modulation to the
    return trunk, then re-runs V1's frozen return heads on modulated trunk.
    """
    ...
    def get_loss(
        ...
        Uses the same loss function as V1 (bucketer.compute_loss) for strong
...
class DriftMonitor:
    ...
    def set_baseline(self, ic_values: dict):
        """Set baseline IC from initial V1 validation."""
        for h in self.horizons:
            if h in ic_values:

...
def train_snapshot_ensemble():
    ...
    """
    log_path = setup_logging(LOG_DIR, "v1e_snapshot_train")
    ...
    ckpt_path = MODEL_DIR / "v1e_snapshot_latest.pt"
    if ckpt_path.exists():
        ...
        if (epoch + 1) % WM_VAL_EVERY == 0 or epoch == WM_TOTAL_EPOCHS - 1:
            ...
            if (epoch + 1) % af_config.shuffled_ic_every == 0:
                ...
                if shuffled_ic > best_shuffled_ic:
                    ...
                    torch.save(ema_model.state_dict(), MODEL_DIR / "v1e_best_ema.pt")
            ...
            if pending_snapshot:
                ...
                if save_snapshot:
                    snap_path = SNAPSHOT_DIR / f"v1_snapshot_{cycle_idx}.pt"
            ...
            state = {
                ...
                "version": "v1e_snapshot_ensemble",
            }
            torch.save(state, MODEL_DIR / "v1e_snapshot_latest.pt")

            ep_path = MODEL_DIR / f"v1e_snapshot_epoch_{epoch+1}.pt"
    ...
    print(f"  Best EMA weights:    {MODEL_DIR / 'v1e_best_ema.pt'}")
    print(f"  Latest checkpoint:   {MODEL_DIR / 'v1e_snapshot_latest.pt'}")

    # -- Snapshot summary ------------------------------------------------------
    existing_snapshots = sorted(SNAPSHOT_DIR.glob("v1_snapshot_*.pt"))
...
if __name__ == "__main__":
    ...
    _ap = argparse.ArgumentParser(description="V1.E Snapshot Ensemble Trainer", add_help=True)

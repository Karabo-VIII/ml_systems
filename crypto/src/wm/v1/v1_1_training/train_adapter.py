...
def compute_context_from_segments(
    ...
    for seg in segments:
        ...
        for i in range(0, len(feats_window) - WM_SEQ_LEN, WM_SEQ_LEN):
            ...
            with torch.no_grad():
                outputs = base_model.forward_train(obs, asset)
...
def main():
    ...
    v1_base_dir = PROJECT_ROOT / "models" / "v1" / "v1_0" / "base"
    base_ckpt_path = v1_base_dir / "v1_0_f13_wm_best_ema.pt"
    if not base_ckpt_path.exists():
        base_ckpt_path = v1_base_dir / "v1_0_f13_wm_weights.pt"
    if not base_ckpt_path.exists():
        print(f"\n  [ERROR] No V1.0 checkpoint found in {v1_base_dir}")
    ...
    for epoch in range(start_epoch, ADAPTER_EPOCHS):
        ...
        for step in pbar:
            ...
            # Run frozen V1 forward (no gradients on base model)
            with torch.no_grad():
...
if __name__ == "__main__":
    ...
    # Adapter inherits actual dims from the frozen backbone, so n_features is used only for logging.
    _ap = argparse.ArgumentParser(description="V1.x Adapter Trainer", add_help=True)

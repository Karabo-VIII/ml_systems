"""
V1.4 World Model Validation -- Comprehensive Diagnostics Suite
...
class WorldModelValidator:
    """Comprehensive diagnostics for V1.4 Transformer-RSSM World Model."""
    ...
    def _load_model(self, model_path: Path):
        ...
        ckpt_prefix = f"v1_4_{feat_tag}{revin_tag}"
    ...
    def run_diagnostics(self):
        print(f"\n{'='*70}")
        print(f"  V1.4 WORLD MODEL COMPREHENSIVE VALIDATION")
    ...
    def run_robust_validation(self, horizon=1):
        ...
        print(f"\n{'='*70}")
        print(f"  V1.4 TRANSFORMER-RSSM ROBUST VALIDATION (Overfitting Detection)")
        print(f"  Model: {self.model_name}")
        ...
        output = {
            "version": "v1.4", "model": self.model_name,
            "timestamp": datetime.now().isoformat(),
...
def main():
    log_path = setup_logging(LOG_DIR, "v1_4_validate")
    parser = argparse.ArgumentParser(description="V1.4 World Model Comprehensive Validation")
    ...
    if args.both:
        ...
        for name, path in [("best_ema", BASE_MODEL_DIR / f"{ckpt_prefix}_wm_best_ema.pt"),
            ...
            if path.exists():
                print(f"\n{'#'*70}")
                print(f"  CHECKPOINT: {name} ({'RevIN' if use_revin else 'no-RevIN'}, {n_features} features)")
                print(f"{'#'*70}")
        ...
        v = WorldModelValidator(BASE_MODEL_DIR / f"{ckpt_prefix}_wm_latest.pt",
                                use_revin=use_revin, n_features=n_features)
        passed = v.run_diagnostics()
        sys.exit(0 if passed else 1)
    else:

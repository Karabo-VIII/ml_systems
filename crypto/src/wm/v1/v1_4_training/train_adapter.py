...
def main():
    ...
    base_ckpt_path = MODEL_DIR / "v1_4_wm_best_ema.pt"
    if not base_ckpt_path.exists():
        base_ckpt_path = MODEL_DIR / "v1_4_wm_weights.pt"
    if not base_ckpt_path.exists():
        print(f"\n  [ERROR] No V1 checkpoint found in {MODEL_DIR}")
        print("  Train V1.4 first: python src/wm/v1/v1_4_training/train_world_model.py")
        teardown_logging()
        return
    ...
    adapter_ckpt_path = MODEL_DIR / "v1_4_adapter_latest.pt"
    start_epoch = 0
    ...
    for epoch in range(start_epoch, ADAPTER_EPOCHS):
        ...
        if (epoch + 1) % ADAPTER_VAL_EVERY == 0 or epoch == ADAPTER_EPOCHS - 1:
            ...
            if val_results["ic_delta_mean"] > best_ic_delta:
                ...
                torch.save(best_state, MODEL_DIR / "v1_4_adapter_best.pt")
                save_marker = " *BEST*"
            ...
            torch.save(ckpt_state, MODEL_DIR / "v1_4_adapter_latest.pt")
    ...
    print(f"  Adapter checkpoint:  {MODEL_DIR / 'v1_4_adapter_latest.pt'}")
    print(f"  Best adapter:        {MODEL_DIR / 'v1_4_adapter_best.pt'}")
...
if __name__ == "__main__":
    ...
    _ap = argparse.ArgumentParser(description="V1.x Adapter Trainer (V1.4)", add_help=True)

...
from diversity_model import DiversityWorldModel, count_parameters
...
def validate(model, val_loader):
    ...
    model.eval()
    metrics = {"rec": [], "kl": [], "regime": [], "total": [], "ncl": []}
    for h in REWARD_HORIZONS:
        metrics[f"ret_{h}"] = []
...
def train_diversity_model(load_backbone: str = None, freeze_backbone: bool = False):
    ...
    """
    log_path = setup_logging(LOG_DIR, "v1_4d_train")
    ...
    ckpt_path = MODEL_DIR / "v1_4d_wm_latest.pt"
    if ckpt_path.exists():
        ...
        if (epoch + 1) % WM_VAL_EVERY == 0 or epoch == DIVERSITY_TOTAL_EPOCHS - 1:
            ...
            if (epoch + 1) % af_config.shuffled_ic_every == 0:
                ...
                if shuffled_ic > best_shuffled_ic:
                    ...
                    torch.save(ema_model.state_dict(), MODEL_DIR / "v1_4d_wm_best_ema.pt")
            ...
            state = {
                ...
                "version": "v1_4d_diversity_ncl_antifragile",
                "n_diversity_heads": DIVERSITY_N_HEADS,
                "ncl_lambda": DIVERSITY_NCL_LAMBDA,
            }
            torch.save(state, MODEL_DIR / "v1_4d_wm_latest.pt")
            torch.save(model.state_dict(), MODEL_DIR / "v1_4d_wm_weights.pt")

            ep_path = MODEL_DIR / f"v1_4d_wm_epoch_{epoch+1}.pt"
    ...
    print(f"  EMA weights:         {MODEL_DIR / 'v1_4d_wm_best_ema.pt'}")
    print(f"  Latest checkpoint:   {MODEL_DIR / 'v1_4d_wm_latest.pt'}")
...
if __name__ == "__main__":
    ...
    # --features is accepted to silence run_all_training.py passing it.
    # Diversity model feature dims are inherited from the backbone checkpoint;
    # the value from --load-backbone determines the actual feature count at runtime.
    parser.add_argument(
        "--features", type=int, default=None,
        choices=sorted(SUPPORTED_FEATURE_COUNTS_V1_4),
        help="Feature count passed by run_all_training.py. Consumed here; actual dims "
             "are determined by the backbone checkpoint loaded via --load-backbone."
    )
    args = parser.parse_args()

    # Echo feature count back for is_complete() detection in run_all_training.py.
    # Diversity model dims are from the backbone; --features is for completion logging only.
    _nf = args.features if args.features is not None else INPUT_DIM

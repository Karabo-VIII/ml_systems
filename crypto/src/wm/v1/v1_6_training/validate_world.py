"""
V1.6 World Model Validation -- Comprehensive Diagnostics Suite
...
directional accuracy (V1.6 tracking metric).
...
class WorldModelValidator:
    """Comprehensive diagnostics for V1.6 'Best of V1' Transformer-RSSM World Model."""
    ...
    def _load_model(self, model_path: Path):
        ...
        ckpt_prefix = f"v1_6_{feat_tag}{revin_tag}"
    ...
    def run_diagnostics(self):
        print(f"\n{'='*70}")
        print(f"  V1.6 WORLD MODEL COMPREHENSIVE VALIDATION")
        print(f"  Model: {self.model_name}")
        print(f"  Time:  {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
        print(f"  Purge Gap: {self.PURGE_GAP_BARS} bars (matches anti_fragile.py)")
        print(f"  TwoHot bins: [{BIN_MIN}, {BIN_MAX}] (raw-return targets, V51)")
        print(f"{'='*70}")
    ...
    def _print_aggregate_report(self, all_results):
        ...
        # Per-asset summary table (V1.6: includes directional accuracy)
        print(f"\n  Per-Asset Summary:")
    ...
    def _check_gates(self, all_results, shuffled_ic=None, loss_ratio=None, ic_h1_for_shic=None):
        ...
        if all_pass:
            ...
            print(f"  V1.6 world model is ready for agent training.")
        else:
    ...
    def run_robust_validation(self, horizon=1):
        ...
        print(f"\n{'='*70}")
        print(f"  V1.6 'BEST OF V1' ROBUST VALIDATION (Overfitting Detection)")
        print(f"  Model: {self.model_name}")
        ...
        output = {
            "version": "v1.6", "model": self.model_name,
            "timestamp": datetime.now().isoformat(),
...
def main():
    log_path = setup_logging(LOG_DIR, "v1_6_validate")
    parser = argparse.ArgumentParser(description="V1.6 'Best of V1' World Model Comprehensive Validation")
    ...
    if args.both:
        ...
        for name, path in [("best_ema", BASE_MODEL_DIR / f"{ckpt_prefix}_wm_best_ema.pt"),
            ...
            if path.exists():
                print(f"\n{'#'*70}")
                print(f"  CHECKPOINT: {name} ({'RevIN' if use_revin else 'no-RevIN'})")
                print(f"{'#'*70}")

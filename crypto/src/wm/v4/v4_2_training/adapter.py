...
Lightweight FiLM-based modulation layer that sits on top of a frozen V4 world model.
Provides regime-adaptive prediction corrections using:
  - Compressed V4 representations (feat bottleneck)
  - Context signals (rolling IC, prediction bias, regime, volatility)
  - Per-horizon scalar scale + shift (bounded, ~15K params)

Design principle: V4 provides deep structural knowledge (slow brain).
V4.x provides current regime adjustments (fast brain). V4.x can only
MODULATE predictions, never override them.

Classes:
  - AdaptiveResidualAdapter: Core FiLM modulation network
  - AdaptedWorldModel: Composite wrapper (frozen V4 + trainable adapter)
...
try:
    ...
    from v4_2_training.settings import (
        REWARD_HORIZONS, ADAPTER_FEAT_DIM, ADAPTER_CONTEXT_DIM,
        ADAPTER_BOTTLENECK, ADAPTER_FILM_HIDDEN,
        ADAPTER_MAX_SCALE_RANGE, ADAPTER_MAX_SHIFT_INIT,
        RETURN_HEAD_DIM, NUM_BINS,
...
class AdaptiveResidualAdapter(nn.Module):
    """
    FiLM-style modulation of V4's return trunk output.

    Takes V4's combined representation (feat) and a context vector,
    produces per-horizon scale and shift to modulate the return trunk.

    Architecture:
      feat [B,T,960] -> Linear(960, bottleneck) -> [B,T,16]
    ...
    def __init__(
        ...
        # Compress V4's 960-dim representation to bottleneck
    ...
    def forward(
        ...
        Args:
            feat:    [B, T, feat_dim] V4's cat(h_seq, z_post)
...
# =============================================================================
# ADAPTED WORLD MODEL (composite: frozen V4 + adapter)
# =============================================================================

class AdaptedWorldModel(nn.Module):
    """
    Composite model: frozen V4 base + trainable adapter.

    Drop-in replacement for MambaWorldModel in validation/inference.
    Runs V4 forward (no gradients), applies adapter modulation to the
    return trunk, then re-runs V4's frozen return heads on modulated trunk.
    """
    ...
    def get_loss(
        ...
        Uses the same loss function as V4 (bucketer.compute_loss) for strong
...
class DriftMonitor:
    ...
    def set_baseline(self, ic_values: dict):
        """Set baseline IC from initial V4 validation."""
        for h in self.horizons:
            if h in ic_values:

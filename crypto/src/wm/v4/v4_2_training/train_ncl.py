"""
V4.2.D Diversity World Model Trainer -- Multi-Head NCL (Anti-Fragile Edition)

Architecture: V4.2 backbone (Mamba-RSSM) + K parallel return heads with NCL
Version: v4_1d_diversity_ncl_antifragile

Key differences from train_world_model.py:
  - Uses DiversityWorldModel instead of MambaWorldModel
  - Logs NCL penalty in epoch summary
  - Can optionally load V4.2 backbone weights (--load-backbone)
  - Saves checkpoints as v4_1d_wm_*.pt
  - Uses same LR schedule, augmentation, and anti-fragile framework as V4

Anti-fragile training features (inherited from V4.2):
...
def train_diversity_model(load_backbone: str = None, freeze_backbone: bool = False, n_features: int = 18):
    """
    Anti-fragile training loop for V4.2.D Diversity World Model.

    Same framework as V4 trainer but with:
    - DiversityWorldModel (K parallel return heads + NCL)
    - NCL penalty logging
    - Optional V4.2 backbone loading
    - v4_1d_* checkpoint naming

    Args:
        load_backbone: path to V4.2 checkpoint to load backbone from (optional)
    ...
    log_path = setup_logging(LOG_DIR, f"v4_1d_{feat_tag}_train")
    torch.set_float32_matmul_precision("medium")

    af_config = AntifragileConfig()

    print("=" * 70)
    print("  V4.D DIVERSITY WORLD MODEL TRAINER (Multi-Head NCL | Anti-Fragile)")
    print("=" * 70)
    ...
    print(f"  Architecture: d_model={WM_D_MODEL}, layers={WM_N_LAYERS}, "
          f"d_state={WM_D_STATE}, expand={WM_EXPAND}")
    ...
    print_antifragile_header("V4.D", af_config)
    ...
    print(f"\n  Initializing V4.2.D Diversity World Model...")
    model = DiversityWorldModel(input_dim=input_dim).to(DEVICE)

    if load_backbone:
        backbone_path = Path(load_backbone)
        if backbone_path.exists():
            print(f"  Loading V4.2 backbone from {backbone_path}")
            ckpt = torch.load(backbone_path, map_location=DEVICE, weights_only=False)
            if "model_state_dict" in ckpt:
                v4_sd = ckpt["model_state_dict"]
            elif "ema_state_dict" in ckpt:
                v4_sd = ckpt["ema_state_dict"]
            else:
                v4_sd = ckpt
            model.load_backbone_from_v4(v4_sd, freeze=freeze_backbone)
        else:
    ...
    ckpt_path = NCL_MODEL_DIR / f"v4_1d_{feat_tag}_wm_latest.pt"
    if ckpt_path.exists():
    ...
    autopsy_path = NCL_MODEL_DIR / f"v4_2d_autopsy.jsonl"
        ...
        if (epoch + 1) % WM_VAL_EVERY == 0 or epoch == DIVERSITY_TOTAL_EPOCHS - 1:
            ...
            if (epoch + 1) % af_config.shuffled_ic_every == 0:
                ...
                if shuffled_ic > best_shuffled_ic:
                    ...
                    }, NCL_MODEL_DIR / f"v4_1d_{feat_tag}_wm_best_ema.pt")
            ...
            torch.save(state, NCL_MODEL_DIR / f"v4_1d_{feat_tag}_wm_latest.pt")
            torch.save(model.state_dict(), NCL_MODEL_DIR / f"v4_1d_{feat_tag}_wm_weights.pt")

            ep_path = NCL_MODEL_DIR / f"v4_1d_{feat_tag}_wm_epoch_{epoch+1}.pt"
    ...
    print("\n" + "=" * 70)
    print("  V4.D DIVERSITY TRAINING COMPLETE (Anti-Fragile)")
    print("=" * 70)
    ...
    print(f"  EMA weights:         {NCL_MODEL_DIR / f'v4_1d_{feat_tag}_wm_best_ema.pt'}")
    print(f"  Latest checkpoint:   {NCL_MODEL_DIR / f'v4_1d_{feat_tag}_wm_latest.pt'}")

    if not gate_passed:
        print("\n  [WARN] Diversity model did not pass validation gate.")
        print("  Do NOT proceed to agent training until gate criteria are met.")
    else:
        print("\n  [OK] V4.2.D Diversity model ready for downstream use.")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="V4.2.D Diversity World Model Trainer")
    parser.add_argument(
        "--load-backbone", type=str, default=None,
        help="Path to V4.2 checkpoint to initialize backbone weights from"
    )
    parser.add_argument(

"""
V4.2 World Model Validation — Comprehensive Diagnostics Suite (Mamba-RSSM + XD Anti-Memorization)
...
class WorldModelValidator:
    """Comprehensive diagnostics for V4.2 Mamba-RSSM World Model."""
    ...
    def __init__(self, model_path: Path = None, n_features: int = 22):
        ...
        self.model = MambaWorldModel(input_dim=input_dim, base_dim=base_dim).to(DEVICE)
    ...
    def run_diagnostics(self):
        print(f"\n{'='*70}")
        print(f"  V4.2 MAMBA-RSSM WORLD MODEL COMPREHENSIVE VALIDATION")
        ...
        for i in tqdm(indices, desc="  Evaluating", leave=False):
            ...
            if i + seq_len + 5 < len(feats):
                ...
                for step in range(1, 6):
                    h_last, z_next, dream_rets, _ = self.model.dream_step(
                        h_last, z_last, asset
                    )
                    z_last = z_next
    ...
    def run_robust_validation(self, horizon=1):
        ...
        print(f"\n{'='*70}")
        print(f"  V4.2 MAMBA-RSSM ROBUST VALIDATION (Overfitting Detection)")
        print(f"  Model: {self.model_name}")
        ...
        output = {
            "version": "v4_1", "model": self.model_name,
            "timestamp": datetime.now().isoformat(),
...
def main():
    parser = argparse.ArgumentParser(description="V4.2 Mamba-RSSM World Model Comprehensive Validation")
    ...
    prefix = f"v4_2_{feat_tag}"
    log_path = setup_logging(LOG_DIR, f"v4_2_{feat_tag}_validate")

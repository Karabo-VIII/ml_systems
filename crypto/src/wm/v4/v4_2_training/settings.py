...
V4.2 Settings -- Mamba Large State Variant (d_state=32)



Ablation: Doubled SSM state dimension (32 vs 16). Larger state space

may capture more complex temporal dynamics at the cost of ~2x SSM VRAM.
...
MODEL_DIR = PROJECT_ROOT / "models" / "wm" / "v4" / "v4_2"

LOG_DIR = PROJECT_ROOT / "logs" / "v4" / "v4_2"
...
WM_D_STATE = 32  # V4.2: larger state (was 16)           # SSM state dimension

WM_N_LAYERS = 2           # Stacked Mamba blocks

WM_EXPAND = 2             # Expansion factor for inner dim
...
try:
    ...
    _assert_canonical(globals(), version_name="v4_2")
except ImportError:

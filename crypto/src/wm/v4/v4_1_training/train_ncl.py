"""
V4.1.D Diversity World Model Trainer -- Multi-Head NCL (Anti-Fragile Edition)

Architecture: V4.1 backbone (Mamba-RSSM) + K parallel return heads with NCL
...
Key differences from train_world_model.py:
  ...
  - Can optionally load V4.1 backbone weights (--load-backbone)
  - Saves checkpoints as v4_1d_wm_*.pt
  - Uses same LR schedule, augmentation, and anti-fragile framework as V4

Anti-fragile training features (inherited from V4.1):
...
def train_diversity_model(load_backbone: str = None, freeze_backbone: bool = False, n_features: int = 18):
    """
    Anti-fragile training loop for V4.1.D Diversity World Model.

    Same framework as V4 trainer but with:
    - DiversityWorldModel (K parallel return heads + NCL)
    - NCL penalty logging
    - Optional V4.1 backbone loading
    - v4_1d_* checkpoint naming

    Args:
        load_backbone: path to V4.1 checkpoint to load backbone from (optional)
    ...
    autopsy_path = NCL_MODEL_DIR / f"v4_1d_autopsy.jsonl"
    ...
    if not gate_passed:
        ...
        print("\n  [OK] V4.1.D Diversity model ready for downstream use.")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="V4.1.D Diversity World Model Trainer")
    parser.add_argument(
        "--load-backbone", type=str, default=None,
        help="Path to V4.1 checkpoint to initialize backbone weights from"
    )
    parser.add_argument(

...
Pipeline:
  1. Load frozen V4.1 base model (best EMA checkpoint)
...
def main(n_features: int = 18):
    ...
    print("=" * 70)
    print("  V4.1.X ADAPTER TRAINER (FiLM Modulation | Rolling Window)")
    print("=" * 70)
    ...
    # -- Load frozen V4.1 base model -------------------------------------------
    base_ckpt_path = BASE_MODEL_DIR / f"v4_1_{feat_tag}_wm_best_ema.pt"
    if not base_ckpt_path.exists():
        base_ckpt_path = BASE_MODEL_DIR / "v4_1_wm_best_ema.pt"
    if not base_ckpt_path.exists():
        base_ckpt_path = BASE_MODEL_DIR / "v4_1_wm_weights.pt"
    if not base_ckpt_path.exists():
        print(f"\n  [ERROR] No V4.1 checkpoint found in {BASE_MODEL_DIR}")
        print("  Train V4.1 first: python src/wm/v4/v4_training/train_world_model.py")
        teardown_logging()
        return
    ...
    if isinstance(state, dict) and "revin_state_dict" in state:
        ...
        print(f"  RevIN loaded from V4.1.1 checkpoint")
    else:
        print(f"  [WARN] No revin_state_dict in V4.1 checkpoint -- using freshly initialized RevIN")
    revin.eval()
    ...
    adapter_ckpt_path = ADAPTER_MODEL_DIR / f"v4_1_{feat_tag}_adapter_latest.pt"
    start_epoch = 0
    ...
    for epoch in range(start_epoch, ADAPTER_EPOCHS):
        ...
        if (epoch + 1) % ADAPTER_VAL_EVERY == 0 or epoch == ADAPTER_EPOCHS - 1:
            ...
            if val_results["ic_delta_mean"] > best_ic_delta:
                ...
                torch.save(best_state, ADAPTER_MODEL_DIR / f"v4_1_{feat_tag}_adapter_best.pt")
                save_marker = " *BEST*"
            ...
            torch.save(ckpt_state, ADAPTER_MODEL_DIR / f"v4_1_{feat_tag}_adapter_latest.pt")
    ...
    # -- Final summary --------------------------------------------------------
    print("\n" + "=" * 70)
    print("  V4.1.X ADAPTER TRAINING COMPLETE")
    print("=" * 70)
    print(f"  Best IC delta:       {best_ic_delta:+.4f}")
    print(f"  Adapter params:      {adapter_params:,}")
    print(f"  Base model hash:     {base_hash}")
    print(f"  Adapter checkpoint:  {ADAPTER_MODEL_DIR / f'v4_1_{feat_tag}_adapter_latest.pt'}")
    print(f"  Best adapter:        {ADAPTER_MODEL_DIR / f'v4_1_{feat_tag}_adapter_best.pt'}")

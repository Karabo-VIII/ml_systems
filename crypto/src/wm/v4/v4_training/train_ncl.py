"""
V4.D Diversity World Model Trainer -- Multi-Head NCL (Anti-Fragile Edition)

Architecture: V4 backbone (Mamba-RSSM) + K parallel return heads with NCL
Version: v4d_diversity_ncl_antifragile
...
Key differences from train_world_model.py:
  ...
  - Can optionally load V4 backbone weights (--load-backbone)
  - Saves checkpoints as v4d_wm_*.pt
  - Uses same LR schedule, augmentation, and anti-fragile framework as V4

Anti-fragile training features (inherited from V4):
...
def train_diversity_model(load_backbone: str = None, freeze_backbone: bool = False, use_revin: bool = False, n_features: int = 37):
    """
    Anti-fragile training loop for V4.D Diversity World Model.

    Same framework as V4 trainer but with:
    - DiversityWorldModel (K parallel return heads + NCL)
    - NCL penalty logging
    - Optional V4 backbone loading
    - v4d_* checkpoint naming

    Args:
        load_backbone: path to V4 checkpoint to load backbone from (optional)
    ...
    log_path = setup_logging(LOG_DIR, f"v4d_{feat_tag}_train")
    ...
    ckpt_path = NCL_MODEL_DIR / "v4d_wm_latest.pt"
    if ckpt_path.exists():
    ...
    autopsy_path = NCL_MODEL_DIR / f"v4d_autopsy.jsonl"
        ...
        if (epoch + 1) % WM_VAL_EVERY == 0 or epoch == DIVERSITY_TOTAL_EPOCHS - 1:
            ...
            if (epoch + 1) % af_config.shuffled_ic_every == 0:
                ...
                if shuffled_ic > best_shuffled_ic:
                    ...
                    torch.save(best_ema_state, NCL_MODEL_DIR / f"v4d{revin_tag}_wm_best_ema.pt")
            ...
            torch.save(state, NCL_MODEL_DIR / "v4d_wm_latest.pt")
            torch.save(model.state_dict(), NCL_MODEL_DIR / "v4d_wm_weights.pt")

            ep_path = NCL_MODEL_DIR / f"v4d_wm_epoch_{epoch+1}.pt"
    ...
    print(f"  EMA weights:         {NCL_MODEL_DIR / f'v4d{revin_tag}_wm_best_ema.pt'}")
    print(f"  Latest checkpoint:   {NCL_MODEL_DIR / 'v4d_wm_latest.pt'}")

    if not gate_passed:
        print("\n  [WARN] Diversity model did not pass validation gate.")
        print("  Do NOT proceed to agent training until gate criteria are met.")
    else:
        print("\n  [OK] V4.D Diversity model ready for downstream use.")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="V4.D Diversity World Model Trainer")
    parser.add_argument("--features", type=int, choices=[13, 18, 30, 34, 37, 41], default=37,
                        help="Feature count: 13/18/30/37 (default: 37)")
    parser.add_argument(
        "--load-backbone", type=str, default=None,
        help="Path to V4 checkpoint to initialize backbone weights from"
    )
    parser.add_argument(

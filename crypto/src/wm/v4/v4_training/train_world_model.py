"""
V4 World Model Trainer -- Mamba-RSSM Multi-Horizon (Anti-Fragile Edition)

Architecture: Mamba Selective State Space Model + RSSM Latents
Version: v4_mamba_rssm_antifragile
...
# expandable_segments=True is the V25-era fix (commit 134039d) that
# eliminated V25's Ep 3 step 0 OOM. Allows segments to grow rather than
# split, better for long-running models with variable memory usage.
# V4 OOM'd at Ep 2 (2026-05-10 v4_validate_2026_05_10c.log) under the older
# max_split_size_mb:128 — switching to expandable_segments per the V25 receipt.
...
# 2026-05-10 fix: add_meta_args was missing from V4 (regression from
# commit e5c17ef "--meta wired across all 14 trainers" sweep that missed
# V4's import). Mirror V1.0's try/except fallback pattern.
...
try:
    ...
    def add_meta_args(parser):
        # Phase 14.7 cleanup: --meta is a NO-OP for V4 (only V25 wires it).
        parser.add_argument("--meta", type=str, default="",
                             help="NO-OP for V4 (only V25 implements meta-learners)")
        parser.add_argument("--meta-distill-alpha", type=float, default=0.0,
                             help="NO-OP for V4 (paired with --meta)")
...
def collate_fn(batch):
    ...
    # Forward-regime labels (V4_FORWARD_REGIME flag; absent when flag is OFF)
...
def _targets_to_device(targets, device):
    ...
    Plain tensors -> .to(device). A nested dict (the V4_FORWARD_REGIME
    `forward_regime_labels` = {"bear","trend","move"}) is recursed one level so its
    inner tensors land on GPU. When V4_FORWARD_REGIME is OFF there is NO nested dict,
...
class CheckpointManager:
    ...
    def __init__(self, save_dir: Path, keep_top_k: int = 3, prefix: str = "v4"):
    ...
    def save(self, model, optimizer, scaler, epoch, val_loss,
             ...
             ema_model=None, revin=None, shic_decline_count=0, n_features=37):
        state = {
            "epoch": epoch,
    ...
    def load_latest(self, model, optimizer, scaler, ema_model=None, revin=None, n_features=37):
        ...
        try:
            ...
            print(f"    Epoch {epoch}, best_val={best_val:.4f}, "
                  ...
                  f"shic_decline={shic_decline}")
            return epoch, best_val, patience, gate, best_shic, shic_decline
...
def train_world_model(use_revin: bool = False, n_features: int = 37, args=None):
    ...
    ckpt_prefix = f"v4_{feat_tag}{revin_tag}{run_tag_str}"

    log_path = setup_logging(LOG_DIR, f"v4_{feat_tag}{run_tag_str}_train")
    ...
    print("=" * 60)
    print("  V4 WORLD MODEL TRAINER (Mamba-RSSM | Anti-Fragile)")
    print("=" * 60)
    print(f"  Device: {DEVICE} | Windows: {IS_WINDOWS}")
    print(f"  Features: {n_features} ({feat_tag}) | Horizons: {REWARD_HORIZONS}")
    print(f"  Ckpt prefix: {ckpt_prefix}")
    print_antifragile_header("V4", af_config)
    ...
    # Activated by env flag V4_FORWARD_REGIME=1 (default "0" = OFF).
    # When OFF: no attachment, no label computation, base path byte-for-byte unchanged.
    # When ON:  attach head to model, pre-compute per-segment forward labels,
    #           pack them into targets via collate_fn -> get_loss adds 0.10 * aux_loss.
    _use_forward_regime = os.environ.get("V4_FORWARD_REGIME", "0") == "1"
    if _use_forward_regime:
        ...
        print("  [V4_FORWARD_REGIME] Pre-computing forward labels on all segments ...")
    ...
    max_epochs = WM_TOTAL_EPOCHS
    if args is not None and getattr(args, "max_epochs", None) is not None:
        max_epochs = args.max_epochs
    print(f"\n  Starting from epoch {start_epoch + 1} / {max_epochs}")
    ...
    for epoch in range(start_epoch, max_epochs):
        ...
        for step in pbar:
            ...
            # FrAug (B003 R2; opt-in) -- applied AFTER seq-shuffle, BEFORE RevIN
            ...
            if upgrade_ctx is not None and upgrade_ctx.use_sam:
                ...
                gn = torch.nn.utils.clip_grad_norm_(clip_params, WM_GRAD_CLIP)
        ...
        print(
            ...
            f"Rec: {avg.get('rec', 0):.4f} | "
            f"KL: {avg.get('kl', 0):.2f} raw:{avg.get('kl_raw', 0):.3f} | "
            f"{ret_str} | "
            f"Reg:{avg.get('regime', 0):.3f} Acc:{avg.get('regime_acc', 0)*100:.0f}% | "
            f"GN: {avg_gn:.2f} | "
            f"Mask: {mask_ratio:.2f} | LR: {lr:.1e} | "
        ...
        if (epoch + 1) % WM_VAL_EVERY == 0 or epoch == WM_TOTAL_EPOCHS - 1:
            ...
            if (epoch + 1) % af_config.shuffled_ic_every == 0:
                ...
                if shuffled_ic > best_shuffled_ic:
                    ...
                    torch.save(best_ema_dict, BASE_MODEL_DIR / f"{ckpt_prefix}_wm_best_ema.pt")
            ...
            ckpt_mgr.save(
                ...
                shic_decline_count=shic_decline_count, n_features=n_features,
            )
...
if __name__ == "__main__":
    ...
    # Force stdout unbuffered so logs survive OOM-kills / driver crashes
    try:
    ...
    parser = argparse.ArgumentParser(description="V4 World Model Trainer (Mamba-RSSM)")
    parser.add_argument("--features", type=int, choices=sorted(SUPPORTED_FEATURE_COUNTS_V4), default=37,
                        help="Feature count: 13/18/30/37 (default: 37 = 30 base + 7 XD)")
    parser.add_argument("--seed", type=int, default=None, help="Random seed for reproducibility")
    parser.add_argument("--max-epochs", type=int, default=None,
                        help="Override WM_EPOCHS for short validation runs (default: settings.WM_EPOCHS)")
    ...
    try:
        ...
        print("\n==== V4 TRAINING CRASHED (captured) ====", flush=True)
        traceback.print_exc()
        # Log CUDA OOM diagnostic if possible
        if torch.cuda.is_available():
            try:
                print(f"  CUDA mem allocated: {torch.cuda.memory_allocated()/1e9:.2f}GB", flush=True)
                print(f"  CUDA mem reserved:  {torch.cuda.memory_reserved()/1e9:.2f}GB", flush=True)
                print(torch.cuda.memory_summary(abbreviated=True), flush=True)
            except Exception:
                pass
        sys.exit(2)
    if not success:
        print("\n  Exiting with gate failure status.", flush=True)
        sys.exit(1)

"""
V4 World Model Validation — Comprehensive Diagnostics Suite (Mamba-RSSM)
...
class WorldModelValidator:
    """Comprehensive diagnostics for V4 Mamba-RSSM World Model."""

    def __init__(self, model_path: Path = None, use_revin: bool = False, n_features: int = 37):
        self.n_features = n_features
        self.feature_list, self.input_dim, self.base_dim = get_feature_config(n_features)
        self.feat_tag = f"f{n_features}"
        self.model = MambaWorldModel(input_dim=self.input_dim).to(DEVICE)
        self.model_name = "unknown"
        self.revin = RevIN(num_features=self.input_dim).to(DEVICE) if use_revin else None
    ...
    def _load_model(self, model_path: Path):
        if model_path is None:
            model_path = BASE_MODEL_DIR / f"v4_{self.feat_tag}{self.revin_tag}_wm_best_ema.pt"
        if not model_path.exists():
            model_path = BASE_MODEL_DIR / f"v4_{self.feat_tag}{self.revin_tag}_wm_latest.pt"
        if not model_path.exists():
        ...
        # LOUD-FAIL on feature-count mismatch before load_state_dict swallows it
        # (V3/V6/V8 have an _assert_dim_match; V4 lacked any guard, so a wrong
        # --features checkpoint would silently emit garbage IC/ShIC). Use the
        # saved n_features (architecture-agnostic; V4's Mamba layer keys differ
        # from V3's obs_encoder so a weight-shape check would be version-fragile).
        if isinstance(ckpt, dict):
            ckpt_nf = ckpt.get("n_features")
            if ckpt_nf is not None and ckpt_nf != self.n_features:
                raise RuntimeError(
                    f"\n  [LOUD FAIL] V4 ckpt {model_path.name} trained at "
                    f"n_features={ckpt_nf} but validator expects {self.n_features}. "
                    f"Re-run with --features {ckpt_nf} (a mismatch silently produces "
                    f"garbage IC/ShIC)."
                )
        ...
        elif isinstance(ckpt, dict) and any(k.startswith("obs_encoder") or k.startswith("mamba_layers") for k in ckpt):
    ...
    def run_diagnostics(self):
        print(f"\n{'='*70}")
        print(f"  V4 MAMBA-RSSM WORLD MODEL COMPREHENSIVE VALIDATION")
    ...
    def _evaluate_asset(self, feats, targets, asset_idx):
        seq_len = WM_SEQ_LEN
        results = {
            "reconstruction": {"total_mse": [], "per_feature_mse": [[] for _ in self.feature_list],
                                "per_feature_r2": [[] for _ in self.feature_list], "cosine_sim": []},
        ...
        for i in tqdm(indices, desc="  Evaluating", leave=False):
            ...
            for fi in range(len(self.feature_list)):
            ...
            # -- 2. Return Prediction (all horizons) -----------------------
            for h in REWARD_HORIZONS:
            ...
            # -- 6. Dream Coherence (Mamba multi-step) ---------------------
            if i + seq_len + 5 < len(feats):
                ...
                for step in range(1, 6):
                    h_last, z_next, gru_hidden_state, dream_rets = self.model.dream_step(
                        h_last, z_last, gru_hidden_state
                    )
                    z_last = z_next
    ...
    def _print_asset_report(self, r, name):
        ...
        for i, feat_name in enumerate(self.feature_list):
        ...
        if any(v != 0.0 for v in r["dream_ic"].values()):
            print(f"\n  DREAM COHERENCE (Mamba SSM multi-step rollout)")
    ...
    def run_robust_validation(self, horizon=1):
        ...
        print(f"\n{'='*70}")
        print(f"  V4 MAMBA-RSSM ROBUST VALIDATION (Overfitting Detection)")
        print(f"  Model: {self.model_name}")
        ...
        output = {
            "version": "v4", "model": self.model_name,
            "timestamp": datetime.now().isoformat(),
...
def main():
    log_path = setup_logging(LOG_DIR, "v4_validate")
    parser = argparse.ArgumentParser(description="V4 Mamba-RSSM World Model Comprehensive Validation")
    parser.add_argument("--features", type=int, choices=[13, 18, 30, 37], default=37,
                        help="Feature count: 13/18/30/37 (must match training config)")
    ...
    ckpt_prefix = f"v4_{feat_tag}{revin_tag}"
    ...
    if args.both:
        ...
        for name, path in [("best_ema", BASE_MODEL_DIR / f"{ckpt_prefix}_wm_best_ema.pt"),
            ...
            if path.exists():
                ...
                print(f"{'#'*70}")
                v = WorldModelValidator(path, use_revin=args.revin, n_features=args.features)
                if args.robust:
        ...
        v = WorldModelValidator(model_path, use_revin=args.revin, n_features=args.features)
        passed = v.run_robust_validation(horizon=args.horizon)
        sys.exit(0 if passed else 1)
    elif args.model:
        v = WorldModelValidator(Path(args.model), use_revin=args.revin, n_features=args.features)
        ...
        v = WorldModelValidator(use_revin=args.revin, n_features=args.features)
        passed = v.run_diagnostics()
        sys.exit(0 if passed else 1)

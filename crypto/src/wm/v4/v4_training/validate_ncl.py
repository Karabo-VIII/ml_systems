...
Validates the V4.D DiversityWorldModel checkpoint from NCL_MODEL_DIR.
...
def load_model(model_path=None, use_revin: bool = False):
    ...
    if model_path is None:
        ckpt_path = NCL_MODEL_DIR / "v4d_ncl_best_ema.pt"
        if not ckpt_path.exists():
            ckpt_path = NCL_MODEL_DIR / "v4d_wm_best_ema.pt"
        if not ckpt_path.exists():
            ckpt_path = NCL_MODEL_DIR / "v4d_ncl_latest.pt"
        if not ckpt_path.exists():

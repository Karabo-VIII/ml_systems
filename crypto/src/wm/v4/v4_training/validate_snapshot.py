"""
V4.E Snapshot Ensemble Validation
...
def run_validation(model_path=None, use_revin: bool = False):
    print(f"\n{'='*65}")
    print(f"  V4.E SNAPSHOT ENSEMBLE VALIDATION")
    ...
    if master_path.exists():
        try:
            ...
            if isinstance(master, dict) and "seed_metrics" in master:
                ...
                print(f"  [OK] Loaded seed ICs from v4e_master.pt")
        except Exception:
            pass
    ...
    if model_path:
        ...
        snapshot_paths = sorted(snap_dir.glob("v4e_seed_*_best.pt"))
        if not snapshot_paths:
            # Fallback to final models (backward compatibility)
            snapshot_paths = sorted(snap_dir.glob("v4_seed_*.pt"))
    else:
    ...
    shic_ratio = shuffled_ic / mean_ic if abs(mean_ic) > 1e-6 else 0.0
...
def main():
    parser = argparse.ArgumentParser(description="V4.E Snapshot Ensemble Validation")

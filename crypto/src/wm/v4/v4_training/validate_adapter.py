"""
V4.X Adapter Validation -- Compares adapted vs base model
...
def load_base_model(model_path=None, use_revin=True):
    """Load MambaWorldModel + RevIN from checkpoint.

    If use_revin=False, RevIN is not created and None is returned in its place.
    """
    if model_path is None:
        model_path = BASE_MODEL_DIR / "v4_wm_best_ema.pt"
    if not model_path.exists():
        model_path = BASE_MODEL_DIR / "v4_wm_latest.pt"
    if not model_path.exists():
    ...
    elif isinstance(ckpt, dict) and any(k.startswith("mamba_layers") for k in ckpt):
        state_dict = ckpt
    else:
        state_dict = ckpt
...
def load_adapter_model(base_model, adapter_path=None):
    ...
    if adapter_path is None:
        for name in ["v4_adapter_best.pt", "v4_adapter_latest.pt"]:
...
def compute_asset_ic(model, revin, feats, targets, asset_idx, use_revin=True):
    ...
    V4 Mamba-SSM: outputs contain h_seq, z_post (RSSM-style latents).
...
def run_validation(base_model_path=None, adapter_path=None, use_revin=True):
    print(f"\n{'='*65}")
    print(f"  V4.X ADAPTER VALIDATION (vs Base)")
    ...
    # V4 Mamba-SSM: recon head maps z_post -> input features.
    # Compare out["recon"] vs obs_norm (RevIN-normalized input, or raw obs if no RevIN).
    rec_mses = []
    ...
    shic_ratio = shuffled_ic / adapted_mean_ic if abs(adapted_mean_ic) > 1e-6 else 0.0
...
def main():
    parser = argparse.ArgumentParser(description="V4.X Adapter Validation vs Base")

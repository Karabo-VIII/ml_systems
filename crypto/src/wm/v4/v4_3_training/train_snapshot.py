"""
V4.E Multi-Seed Ensemble Trainer -- K independent full training runs.

Architecture: Mamba-RSSM (Mamba Selective State Space Model + RSSM Latents)
...
def save_master_checkpoint(seeds, completed, seed_metrics, feat_tag: str = "f18"):
    ...
    state = {
        ...
        "version": "v4_3e_multi_seed_ensemble",
    }
    torch.save(state, ENSEMBLE_MODEL_DIR / f"v4_3e_{feat_tag}_master.pt")
...
def train_single_seed(
    ...
    """
    Train one full model with a specific seed. Returns metrics dict.
    ...
    Args:
        ...
        base_dim:        Number of base features for decoder
        feat_tag:        Feature tag string (e.g. "f18" or "f13")
    ...
    print(f"{'=' * 70}")
    print(f"  Architecture: d_model={WM_D_MODEL}, layers={WM_N_LAYERS}, d_state={WM_D_STATE}")
    ...
    # -- Optimizer ----------------------------------------------------------------
    all_params = list(model.parameters()) + list(revin.parameters())
    ...
    start_epoch = 0
    ckpt_path = ENSEMBLE_MODEL_DIR / f"v4_3e_{feat_tag}_seed_{seed_idx}_latest.pt"
    if ckpt_path.exists():
    ...
    autopsy_path = ENSEMBLE_MODEL_DIR / f"v4_3e_seed_{seed_idx}_autopsy.jsonl"
    ...
    for epoch in range(start_epoch, WM_TOTAL_EPOCHS):
        ...
        for step in pbar:
            ...
            # NaN detection
            has_nan = False
            for comp_val in loss_dict.values():
        ...
        if (epoch + 1) % WM_VAL_EVERY == 0 or epoch == WM_TOTAL_EPOCHS - 1:
            ...
            if (epoch + 1) % af_config.shuffled_ic_every == 0:
                ...
                if shuffled_ic > best_shuffled_ic:
                    ...
                    }, ENSEMBLE_MODEL_DIR / f"v4_3e_{feat_tag}_seed_{seed_idx}_best.pt")
            ...
            state = {
                ...
                "seed": seed,
                "seed_idx": seed_idx,
                "version": "v4_3e_seed_training",
                "n_features": INPUT_DIM,
            }
            torch.save(state, ENSEMBLE_MODEL_DIR / f"v4_3e_{feat_tag}_seed_{seed_idx}_latest.pt")

            # Keep top-3 epoch checkpoints
            ep_path = ENSEMBLE_MODEL_DIR / f"v4_3e_{feat_tag}_seed_{seed_idx}_epoch_{epoch+1}.pt"
    ...
    final_path = ENSEMBLE_MODEL_DIR / f"v4_3_{feat_tag}_seed_{seed_idx}.pt"
    torch.save({
        "model_state_dict": ema_model.state_dict(),
        "revin_state_dict": revin.state_dict(),
    }, final_path)
    ...
    # Cleanup per-seed training checkpoint (keep final model + best EMA only)
    latest_ckpt = ENSEMBLE_MODEL_DIR / f"v4_3e_{feat_tag}_seed_{seed_idx}_latest.pt"
    ...
    for ep_ckpt in ENSEMBLE_MODEL_DIR.glob(f"v4_3e_{feat_tag}_seed_{seed_idx}_epoch_*.pt"):
        try:
...
def train_ensemble(n_features: int = 18):
    """
    Multi-seed ensemble orchestrator. Loads data once, then trains
    K independent models back-to-back with different seeds.
    """
    feature_list, input_dim, base_dim = get_feature_config(n_features)
    feat_tag = f"f{n_features}"
    log_path = setup_logging(LOG_DIR, f"v4_3e_{feat_tag}_ensemble_train")
    ...
    print("=" * 70)
    print("  V4.3.E MULTI-SEED ENSEMBLE TRAINER")
    print("=" * 70)
    ...
    print_antifragile_header("V4.E", af_config)
    ...
    master_path = ENSEMBLE_MODEL_DIR / f"v4_3e_{feat_tag}_master.pt"
    ...
    existing_seeds = sorted(ENSEMBLE_MODEL_DIR.glob(f"v4_3_{feat_tag}_seed_*.pt"))
...
if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description="V4.E Multi-Seed Ensemble Trainer")
    parser.add_argument(
        "--features", type=int, choices=[13, 18], default=18,
        help="Number of input features: 13 (base only) or 18 (base + cross-asset). Default: 19"
    )

"""
V4.3 World Model Trainer -- Mamba-RSSM + XD Anti-Memorization

Architecture: Mamba Selective State Space Model + RSSM Latents
Version: v4_3_mamba_rssm_antifragile
...
class CheckpointManager:
    ...
    def __init__(self, save_dir: Path, keep_top_k: int = 3, prefix: str = "v4_3_f18"):
    ...
    def save(self, model, optimizer, scaler, epoch, val_loss,
        ...
        state = {
            ...
            "version": f"v4_3_{self.prefix}_mamba_rssm_antifragile",
        }
...
def train_world_model(n_features: int = 18, use_revin: bool = False):
    """
    Anti-fragile training loop for V4.3 Mamba-RSSM.
    ...
    ckpt_prefix = f"v4_3_{feat_tag}"
    log_path = setup_logging(LOG_DIR, f"v4_3_{feat_tag}_train")
    ...
    print("=" * 60)
    print("  V4.3 WORLD MODEL TRAINER (Mamba-RSSM | Anti-Fragile)")
    print("=" * 60)
    ...
    print_antifragile_header("V4.3", af_config)
...
if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description="V4.3 World Model Trainer")

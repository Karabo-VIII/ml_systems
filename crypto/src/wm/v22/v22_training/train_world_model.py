#!/usr/bin/env python
"""V22 Training -- iTransformer (Liu et al., ICLR 2024).

Channel-tokenized cross-feature attention. Cleanest fix for V12's structural
synchronization issue — feature tokens don't need timestamp alignment across
assets, so cross-asset modeling is implicit in cross-feature attention.
...
_wm_spec = importlib.util.spec_from_file_location(
    "v22_world_model", str(Path(__file__).resolve().parent / "world_model.py")
)
_wm_mod = importlib.util.module_from_spec(_wm_spec)
_wm_spec.loader.exec_module(_wm_mod)
iTransformerWorldModel = _wm_mod.iTransformerWorldModel
...
def save_checkpoint(model, ema_model, optimizer, epoch, val_loss, patience,
    ...
    if ema_only:
        torch.save({"model_state_dict": ema_model.state_dict(),
                    "n_features": n_features, "version": "v22"}, path)
    else:
        torch.save({
...
def load_latest(model, ema_model, optimizer, ckpt_dir, n_features, run_tag_str: str = ""):
    latest_path = ckpt_dir / f"v22_f{n_features}{run_tag_str}_wm_latest.pt"
    ema_path = ckpt_dir / f"v22_f{n_features}{run_tag_str}_wm_best_ema.pt"
...
def run_training(n_features=None, resume=True, args=None):
    ...
    # 2026-05-09 reproducibility: pin random seed (mirror of V25 fix).
    # V25's regime classifier had stochastic basin-selection bug; V22 doesn't
    # have regime gating but pinning seed ensures reproducible IC across runs.
    ...
    print("=" * 70)
    print("  V22 TRAINING -- iTransformer (Liu et al., ICLR 2024)")
    print(f"  Features: {input_dim} | d_model: {WM_D_MODEL} | n_layers: {WM_N_LAYERS} | Device: {DEVICE}")
    print(f"  Channel-tokenized: cross-feature attention (no sync requirement)")
    print("=" * 70)
    ...
    # 2026-05-21 oracle validation: with last-bar supervision (Timer-XL/
    # TimesFM pattern), each sample contributes ONE supervised position. To
    # preserve effective supervision count, use stride=1 sliding windows
    # (consecutive bars each become a "last bar" target). Default stride
    # would be seq_len//4 = 24, giving 1/24 the supervision density.
    try:
    ...
    model = iTransformerWorldModel(input_dim=input_dim).to(DEVICE)
    ema_model = iTransformerWorldModel(input_dim=input_dim).to(DEVICE)
    ...
    if getattr(args, "autopsy", False):
        try:
            ...
            autopsy = AutopsyMode(
                model,
                log_dir=Path(__file__).resolve().parents[4] / "logs" / "v22",
                run_tag=f"v22_f{input_dim}{run_tag_str}",
    ...
    for epoch in range(start_epoch, max_epochs):
        ...
        for step in pbar:
            ...
            # AutopsyMode (opt-in; safe under failure)
            ...
            # Re-uses base_outputs captured from get_loss above — no re-forward needed.
            if meta_rt.flags:
        ...
        # Loss component breakdown (already computed; surface for diagnosis)
        loss_parts = " ".join([
        ...
        # Sign-flip early-warning: if IC < 0 AND IC(-pred) > +0.10, alert
        signflip_alerts = []
        ...
        # Magnitude sanity (h=1): pred std should be within 0.5x-2x of real std
        ...
        if (epoch + 1) % SHUFFLED_IC_CHECK_INTERVAL == 0:
            ...
            # 2026-05-10 fix: gate direction was inverted. Per CLAUDE.md
            # validation gates + anti_fragile.py:826-845, the gate is
            # ShIC/IC > 0.3 = PASS (signal generalizes; ratio approaches 1.0).
            # ShIC/IC < 0.3 = FAIL (memorization; ratio collapses to 0).
            # This was the same inverted-reading bug that drove the V1
            # capacity-scaling rollback. V25 trainer was fixed in commit
            # 0c1f539; V22 was missed in that sweep. See
            # docs/V1_CAPACITY_SCALING_AUDIT_2026_05_10.md for full receipt.
        ...
        if val_loss < best_val:
            best_val = val_loss
            patience = 0
            ckpt = MODEL_DIR / f"v22_f{input_dim}{run_tag_str}_wm_best_ema.pt"
        ...
        latest = MODEL_DIR / f"v22_f{input_dim}{run_tag_str}_wm_latest.pt"
        save_checkpoint(model, ema_model, optimizer, epoch + 1, val_loss,
                        patience, gate, best_shic, shic_decline, input_dim, latest)
        if (epoch + 1) % 5 == 0:
            ep = MODEL_DIR / f"v22_f{input_dim}{run_tag_str}_wm_epoch_{epoch + 1}.pt"
            save_checkpoint(model, ema_model, optimizer, epoch + 1, val_loss,
                            patience, gate, best_shic, shic_decline, input_dim, ep)

        # 2026-05-09 OOM fix (mirror of V25): flush CUDA cache at end of each
        # epoch to prevent fragmentation-induced OOM during long runs.
        if torch.cuda.is_available():
    ...
    if autopsy is not None:
        try:
            try:
                ...
                pass
            # V22 round-10 has patch_embed (memorization-vector replacement)
            try:
                if hasattr(model, "patch_embed") and model.patch_embed is not None:
                    autopsy.embedding_health_probe(model.patch_embed,
                                                     name="patch_embed")
            except Exception:
                pass
            autopsy.close()
    ...
    print("\n  TRAINING COMPLETE (Anti-Fragile) -- V22")
    return True


if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="V22 iTransformer Training")
    parser.add_argument("--features", type=int, default=29,
                        choices=sorted(SUPPORTED_FEATURE_COUNTS_V22))
    parser.add_argument("--resume", action="store_true")
    parser.add_argument("--max-epochs", type=int, default=None,
                        help="Override WM_EPOCHS (useful for short autopsy runs).")
    parser.add_argument("--seed", type=int, default=42,
                        help="Random seed (default 42, pinned for reproducibility).")

"""
V8.D NCL Diversity Validation -- Per-head IC + ensemble IC + diversity metrics

Validates the V8.D DiversityWorldModel checkpoint from NCL_MODEL_DIR.
...
def load_model(model_path=None, use_revin: bool = False):
    ...
    if model_path is None:
        ckpt_path = NCL_MODEL_DIR / "v8d_ncl_best_ema.pt"
        if not ckpt_path.exists():
            ckpt_path = NCL_MODEL_DIR / "v8d_wm_best_ema.pt"
        if not ckpt_path.exists():
    ...
    print(f"  [OK] V8.D loaded  |  heads={K}  |  NCL lambda={model.ncl_lambda}")
...
def compute_diversity_metrics(head_preds, ensemble_reals, K):
    ...
    Returns:
        corr_matrix: [K, K] pairwise Pearson correlations
...
def print_report(asset_results, ensemble_preds, ensemble_reals, head_preds,
    ...
    print(f"\n{'='*70}")
    print(f"  V8.D NCL DIVERSITY VALIDATION")
    ...
    if all_pass:
        ...
        print(f"  V8.D NCL model is validated.")
    else:
        print(f"  VERDICT: GATE(S) FAILED")
        print(f"  V8.D NCL model needs more training or tuning.")
    print(f"  {'='*40}")
...
def main():
    parser = argparse.ArgumentParser(description="V8.D NCL Diversity Validation")
    ...
    print(f"\n{'='*70}")
    print(f"  V8.D NCL DIVERSITY VALIDATOR")

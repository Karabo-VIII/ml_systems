"""
V8 World Model Validation -- Comprehensive Diagnostics Suite (Neural ODE)

Run this BEFORE proceeding to agent training.
Evaluates: reconstruction, return prediction (all horizons), latent health,
imagination quality, regime classification, per-asset breakdown.

NOTE: V8 has NO dream_step (no dream coherence test). Continuous-time
dynamics are evaluated via reconstruction under masking (imagination).
...
class WorldModelValidator:
    """Comprehensive diagnostics for V8 Neural ODE World Model."""
    ...
    def __init__(self, model_path: Path = None, use_revin: bool = False,
        ...
        self.model = NeuralODEWorldModel().to(DEVICE)
    ...
    def _load_model(self, model_path: Path):
        # Cohort-harmonized 2026-05-03: ckpt includes f{n_features} + run_tag.
        prefix = f"v8_f{self.n_features}{self.revin_tag}{self.run_tag_str}"
        ...
        if not model_path.exists():
            ...
            for p in (BASE_MODEL_DIR / f"v8{self.revin_tag}_wm_best_ema.pt",
                       BASE_MODEL_DIR / f"v8{self.revin_tag}_wm_latest.pt"):
                if p.exists():
                    model_path = p
        ...
        # 2026-05-03 LOUD-FAIL dim check (catches silent wrong-feature loads).
        ...
        elif isinstance(ckpt, dict) and any(k.startswith("initial_encoder") or k.startswith("dynamics") for k in ckpt):
    ...
    def _assert_dim_match(self, sd: dict, ckpt_name: str) -> None:
        ...
        for k, v in sd.items():
            kl = k.lower()
            # V8 uses initial_encoder rather than obs_encoder
            if (kl.startswith("obs_encoder") or kl.startswith("initial_encoder")) and kl.endswith("weight") and v.ndim == 2:
                ckpt_input = int(v.shape[1])
                expected = self.n_features
                if ckpt_input == expected:
                    return
                emb = sd.get("asset_embedding.weight")
                if emb is not None and emb.ndim == 2 and ckpt_input - emb.shape[1] == expected:
                    return
                raise RuntimeError(
                    f"\n  [LOUD FAIL] DIM MISMATCH on ckpt {ckpt_name}\n"
                    f"  encoder weight shape[1]={ckpt_input}, expected n_features={expected}.\n"
                    f"  Pass --features N matching the training run, or --model <path> "
                    f"with a ckpt at n_features={expected}. Refusing to load -- silent "
                    f"wrong weights would emit garbage IC/ShIC."
                )
        return
    ...
    def run_diagnostics(self):
        print(f"\n{'='*70}")
        print(f"  V8 NEURAL ODE WORLD MODEL COMPREHENSIVE VALIDATION")
    ...
    def _evaluate_asset(self, feats, targets, asset_idx):
        ...
        for i in tqdm(indices, desc="  Evaluating", leave=False):
            ...
            h_seq = outputs["h_seq"]
            obs_for_ode = outputs["obs_for_ode"]
            t_span = outputs["t"]
            ...
            # -- 4. Dynamics Regularity ------------------------------------
            # Compute ||f(h_t, t, obs_t)||^2 at sampled timesteps
            dyn_norm = self.model.dynamics_regularization(
                h_seq, obs_for_ode, t_span
            ).item()
            results["dynamics"]["norm_sq"].append(dyn_norm)

            # -- 5. Imagination (varying mask levels) ----------------------
    ...
    def _aggregate_results(self, raw):
        ...
        # Dynamics regularity
        agg["dynamics_norm_sq"] = float(np.mean(raw["dynamics"]["norm_sq"]))
        agg["dynamics_norm_std"] = float(np.std(raw["dynamics"]["norm_sq"]))
    ...
    def _print_asset_report(self, r, name):
        ...
        # Dynamics Regularity (V8-specific)
        print(f"\n  DYNAMICS REGULARITY (Neural ODE)")
        print(f"    ||f(h,t,obs)||^2:  {r['dynamics_norm_sq']:.6f} +/- {r['dynamics_norm_std']:.6f}")
        dyn_status = "PASS" if r['dynamics_norm_sq'] < 100.0 else "WARN" if r['dynamics_norm_sq'] < 500.0 else "FAIL"
        print(f"    Status:            {dyn_status} (lower = smoother dynamics)")
    ...
    def _print_aggregate_report(self, all_results):
        ...
        # Dynamics
        avg_dyn = np.mean([r["dynamics_norm_sq"] for r in all_results.values()])
        print(f"\n  Dynamics Regularity:")
        print(f"    Avg ||f||^2:     {avg_dyn:.6f}")
        ...
        print(f"\n  Per-Asset Summary:")
        print(f"    {'Asset':<10} {'RecMSE':>8} {'IC(1)':>8} {'IC(4)':>8} {'IC(16)':>8} {'IC(64)':>8} {'KL':>6} {'Dir(1)':>7} {'DynNorm':>8}")
        print(f"    {'-'*78}")
    ...
    def _save_results(self, all_results, gate_pass):
        ...
        for name, r in all_results.items():
            output["results"][name] = {
                "rec_mse": r["rec_mse"],
                "kl_mean": r["kl_mean"],
                "dynamics_norm_sq": r["dynamics_norm_sq"],
                "returns": {str(h): {
                    "ic": r["returns"][h]["ic"],
    ...
    def run_robust_validation(self, horizon=1):
        ...
        print(f"\n{'='*70}")
        print(f"  V8 NEURAL ODE ROBUST VALIDATION (Overfitting Detection)")
        print(f"  Model: {self.model_name}")
        ...
        output = {
            "version": "v8", "architecture": "neural_ode",
            "model": self.model_name,
            "timestamp": datetime.now().isoformat(),
...
def main():
    log_path = setup_logging(LOG_DIR, "v8_validate")
    parser = argparse.ArgumentParser(description="V8 Neural ODE World Model Comprehensive Validation")
    ...
    parser.add_argument("--features", type=int, default=None,
                        help="Feature count of the trained ckpt. Default: settings.INPUT_DIM. "
                             "MUST match training --features or model load uses wrong-shape weights.")
    parser.add_argument("--run-tag", type=str, default="",
                        help="Run-tag suffix used at training (default empty).")
    ...
    prefix = f"v8_f{n_features}{revin_tag}{run_tag_str}"

    def _resolve(suffix: str) -> Path:
        """Resolve to new prefix; fall back to legacy bare name if missing."""
        new = BASE_MODEL_DIR / f"{prefix}_wm_{suffix}.pt"
        if new.exists():
            return new
        legacy = BASE_MODEL_DIR / f"v8{revin_tag}_wm_{suffix}.pt"
        if legacy.exists():
            print(f"  [INFO] using legacy ckpt {legacy.name}")
            return legacy
        return new  # caller will see missing-file error

    if args.both:
        print(f"\n  Running validation on BOTH checkpoints (prefix={prefix})...\n")
        for name, path in [("best_ema", _resolve("best_ema")),
                           ("latest",   _resolve("latest"))]:
            if path.exists():
        ...
        v = WorldModelValidator(_resolve("latest"), use_revin=args.revin,
                                 n_features=n_features, run_tag=args.run_tag)
        passed = v.run_diagnostics()
        sys.exit(0 if passed else 1)
    else:

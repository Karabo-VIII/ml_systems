"""
V8.X Adapter Validation -- Compares adapted vs base model (Neural ODE-RSSM)
...
V8-SPECIFIC NOTES:
  - Base model: NeuralODEWorldModel (continuous-time RK4 ODE dynamics)
  - Adapter feat: cat(h_seq, z_post) = [B, T, 256+576=832]
  - Output dict: h_seq [B,T,256], z_post [B,T,576], ret_trunk [B,T,384]
  - Checkpoint prefix: v8x_adapter_*; base: v8_wm_best_ema.pt
"""
import torch
...
def load_base_model(model_path=None, use_revin=True):
    """Load NeuralODEWorldModel + RevIN from checkpoint.

    If use_revin=False, RevIN is not created and None is returned in its place.
    """
    if model_path is None:
        model_path = BASE_MODEL_DIR / "v8_wm_best_ema.pt"
    if not model_path.exists():
        model_path = BASE_MODEL_DIR / "v8_wm_latest.pt"
    if not model_path.exists():
...
def load_adapter_model(base_model, adapter_path=None):
    ...
    if adapter_path is None:
        for name in ["v8_adapter_best.pt", "v8_adapter_latest.pt"]:
...
def compute_asset_ic(model, revin, feats, targets, asset_idx, use_revin=True):
    ...
    V8-SPECIFIC: adapter feat = cat(h_seq, z_post); forward_train runs ODE
    solver (RK4) to produce h_seq; return_logits at [B, T, NUM_BINS].
    """
    seq_len = WM_SEQ_LEN
...
def run_validation(base_model_path=None, adapter_path=None, use_revin=True):
    print(f"\n{'='*65}")
    print(f"  V8.X ADAPTER VALIDATION (vs Base)")
...
def main():
    parser = argparse.ArgumentParser(description="V8.X Adapter Validation vs Base")

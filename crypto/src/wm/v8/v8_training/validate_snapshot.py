"""
V8.E Snapshot Ensemble Validation
...
def run_validation(model_path=None, use_revin: bool = False):
    print(f"\n{'='*65}")
    print(f"  V8.E SNAPSHOT ENSEMBLE VALIDATION")
    ...
    if master_path.exists():
        try:
            ...
            if isinstance(master, dict) and "seed_metrics" in master:
                ...
                print(f"  [OK] Loaded seed ICs from v8e_master.pt")
        except Exception:
            pass
    ...
    if model_path:
        ...
        snapshot_paths = sorted(snap_dir.glob("v8e_seed_*_best.pt"))
        if not snapshot_paths:
            # Fallback to final models (backward compatibility)
            snapshot_paths = sorted(snap_dir.glob("v8_seed_*.pt"))
    else:
...
def main():
    parser = argparse.ArgumentParser(description="V8.E Snapshot Ensemble Validation")

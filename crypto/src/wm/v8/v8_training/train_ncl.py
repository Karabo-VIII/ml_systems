"""
V8.D Diversity World Model Trainer -- Multi-Head NCL (Anti-Fragile Edition)

Architecture: V8 backbone (Neural ODE-RSSM) + K parallel return heads with NCL
Version: v8d_diversity_ncl_antifragile
...
Key differences from train_world_model.py:
  ...
  - Can optionally load V8 backbone weights (--load-backbone)
  - Saves checkpoints as v8d_wm_*.pt
  - Uses same LR schedule, augmentation, and anti-fragile framework as V8

Anti-fragile training features (inherited from V8):
...
def train_diversity_model(load_backbone: str = None, freeze_backbone: bool = False, use_revin: bool = False, n_features: int = None):
    """
    Anti-fragile training loop for V8.D Diversity World Model.
    ...
    - Optional V8 backbone loading
    - v8d_* checkpoint naming

    Args:
        load_backbone: path to V8 checkpoint to load backbone from (optional)
        freeze_backbone: if True, freeze backbone and only train diversity heads
    """
    log_path = setup_logging(LOG_DIR, "v8d_train")
    ...
    ckpt_path = NCL_MODEL_DIR / "v8d_wm_latest.pt"
    if ckpt_path.exists():
    ...
    autopsy_path = NCL_MODEL_DIR / f"v8d_autopsy.jsonl"
        ...
        if (epoch + 1) % WM_VAL_EVERY == 0 or epoch == DIVERSITY_TOTAL_EPOCHS - 1:
            ...
            if (epoch + 1) % af_config.shuffled_ic_every == 0:
                ...
                if shuffled_ic > best_shuffled_ic:
                    ...
                    torch.save(best_ema_state, NCL_MODEL_DIR / f"v8d{revin_tag}_wm_best_ema.pt")
            ...
            torch.save(state, NCL_MODEL_DIR / "v8d_wm_latest.pt")
            torch.save(model.state_dict(), NCL_MODEL_DIR / "v8d_wm_weights.pt")

            ep_path = NCL_MODEL_DIR / f"v8d_wm_epoch_{epoch+1}.pt"
    ...
    print(f"  EMA weights:         {NCL_MODEL_DIR / f'v8d{revin_tag}_wm_best_ema.pt'}")
    print(f"  Latest checkpoint:   {NCL_MODEL_DIR / 'v8d_wm_latest.pt'}")

    if not gate_passed:
        print("\n  [WARN] Diversity model did not pass validation gate.")
        print("  Do NOT proceed to agent training until gate criteria are met.")
    else:
        print("\n  [OK] V8.D Diversity model ready for downstream use.")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="V8.D Diversity World Model Trainer")
    parser.add_argument(
        "--load-backbone", type=str, default=None,
        help="Path to V8 checkpoint to initialize backbone weights from"
    )
    parser.add_argument(

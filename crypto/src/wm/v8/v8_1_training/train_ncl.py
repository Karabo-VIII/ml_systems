"""
V8.1.D Diversity World Model Trainer -- Multi-Head NCL (Anti-Fragile Edition)

Architecture: V8.1 backbone (Neural ODE-RSSM) + K parallel return heads with NCL
...
Key differences from train_world_model.py:
  ...
  - Can optionally load V8.1 backbone weights (--load-backbone)
  - Saves checkpoints as v8_1d_wm_*.pt
  - Uses same LR schedule, augmentation, and anti-fragile framework as V8

Anti-fragile training features (inherited from V8.1):
...
def train_diversity_model(load_backbone: str = None, freeze_backbone: bool = False,
                          n_features: int = 18):
    """
    Anti-fragile training loop for V8.1.D Diversity World Model.
    ...
    - Optional V8.1 backbone loading
    - v8_1d_* checkpoint naming

    Args:
        load_backbone: path to V8.1 checkpoint to load backbone from (optional)
    ...
    autopsy_path = NCL_MODEL_DIR / f"v8_1d_autopsy.jsonl"
    ...
    if not gate_passed:
        ...
        print("\n  [OK] V8.1.D Diversity model ready for downstream use.")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="V8.1.D Diversity World Model Trainer")
    parser.add_argument(
        "--load-backbone", type=str, default=None,
        help="Path to V8.1 checkpoint to initialize backbone weights from"
    )
    parser.add_argument(

"""
V8.1 World Model Validation -- Neural ODE + XD Anti-Memorization
...
Usage:
    python validate_world.py --features 18      # Full 18 features (default)
    python validate_world.py --features 14      # Base 14 features only
...
class WorldModelValidator:
    """Comprehensive diagnostics for V8.1 Neural ODE World Model."""
    ...
    def __init__(self, model_path: Path = None, n_features: int = 22):
        ...
        self.model = NeuralODEWorldModel(input_dim=input_dim, base_dim=base_dim).to(DEVICE)
    ...
    def _load_model(self, model_path: Path):
        ...
        if isinstance(ckpt, dict) and "revin_state_dict" in ckpt:
            self.revin = RevIN(num_features=self.input_dim).to(DEVICE)
    ...
    def run_diagnostics(self):
        print(f"\n{'='*70}")
        print(f"  V8.1 NEURAL ODE WORLD MODEL COMPREHENSIVE VALIDATION")
    ...
    def run_robust_validation(self, horizon=1):
        ...
        print(f"\n{'='*70}")
        print(f"  V8.1 NEURAL ODE ROBUST VALIDATION (Overfitting Detection)")
        print(f"  Model: {self.model_name}")
        ...
        output = {
            "version": "v8_1", "architecture": "neural_ode",
            "model": self.model_name,
            "timestamp": datetime.now().isoformat(),
...
def main():
    parser = argparse.ArgumentParser(description="V8.1 Neural ODE World Model Comprehensive Validation")
    ...
    parser.add_argument("--features", type=int, choices=[13, 18, 30, 37], default=13,
                        help="Number of features: 13 (base only) or 18 (full + XD defenses)")
    args = parser.parse_args()

    feat_tag = f"f{args.features}"
    prefix = f"v8_1_{feat_tag}"
    log_path = setup_logging(LOG_DIR, f"v8_1_{feat_tag}_validate")

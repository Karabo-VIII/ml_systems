...
Lightweight FiLM-based modulation layer that sits on top of a frozen V8 world model.
Provides regime-adaptive prediction corrections using:
  - Compressed V8 representations (feat bottleneck)
  - Context signals (rolling IC, prediction bias, regime, volatility)
  - Per-horizon scalar scale + shift (bounded, ~15K params)

Design principle: V8 provides deep structural knowledge (slow brain).
V8.x provides current regime adjustments (fast brain). V8.x can only
MODULATE predictions, never override them.

Classes:
  - AdaptiveResidualAdapter: Core FiLM modulation network
  - AdaptedWorldModel: Composite wrapper (frozen V8 + trainable adapter)
...
class AdaptiveResidualAdapter(nn.Module):
    """
    FiLM-style modulation of V8's return trunk output.

    Takes V8's combined representation (feat) and a context vector,
    ...
    def __init__(
        ...
        # Compress V8's 832-dim representation to bottleneck
    ...
    def forward(
        ...
        Args:
            feat:    [B, T, feat_dim] V8's cat(h_seq, z_post)
...
# =============================================================================
# ADAPTED WORLD MODEL (composite: frozen V8 + adapter)
# =============================================================================

class AdaptedWorldModel(nn.Module):
    """
    Composite model: frozen V8 base + trainable adapter.

    Drop-in replacement for NeuralODEWorldModel in validation/inference.
    Runs V8 forward (no gradients), applies adapter modulation to the
    return trunk, then re-runs V8's frozen return heads on modulated trunk.
    """
    ...
    def get_loss(
        ...
        Uses the same loss function as V8 (bucketer.compute_loss) for strong
...
class DriftMonitor:
    ...
    def set_baseline(self, ic_values: dict):
        """Set baseline IC from initial V8 validation."""
        for h in self.horizons:
            if h in ic_values:

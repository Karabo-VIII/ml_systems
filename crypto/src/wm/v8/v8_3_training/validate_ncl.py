...
Validates the V8.3.D DiversityWorldModel checkpoint from NCL_MODEL_DIR.
...
def load_model(model_path=None):
    ...
    if model_path is None:
        ckpt_path = NCL_MODEL_DIR / "v8_1d_ncl_best_ema.pt"
        if not ckpt_path.exists():
            ckpt_path = NCL_MODEL_DIR / "v8_1d_wm_best_ema.pt"
        if not ckpt_path.exists():
            ckpt_path = NCL_MODEL_DIR / "v8_1d_wm_latest.pt"
    else:

"""
V8.x Adapter Trainer -- Rolling Window Adaptation

Trains a lightweight FiLM adapter on top of a frozen V8 world model.
The adapter learns regime-specific corrections to V8's return predictions.

Pipeline:
  1. Load frozen V8.3 base model (best EMA checkpoint)
  2. Load data, split train/val via walk-forward
  3. Build context vector from TRAINING data (not val -- prevents leakage)
  4. Train adapter on-the-fly: each step runs frozen V8 + adapter forward
  5. Validate: adapted IC vs base IC per horizon
  6. Save adapter if it improves over base

On-the-fly design (no pre-caching):
  - Each training step runs frozen V8 forward (no_grad) then adapter forward
  - Avoids 40GB+ memory for cached representations
  - V8 forward under no_grad is fast (~20ms per batch on RTX 4060)
...
def compute_context_from_segments(
    base_model: NeuralODEWorldModel,
    segments: list,
    device: torch.device,
...
def main(n_features: int = 18):
    ...
    print("=" * 70)
    print("  V8.3.X ADAPTER TRAINER (FiLM Modulation | Rolling Window)")
    print("=" * 70)
    print(f"  Device:         {DEVICE}")
    print(f"  Features:       {input_dim} ({feat_tag})")
    print(f"  Base model:     NeuralODEWorldModel (frozen)")
    ...
    # -- Setup logging --------------------------------------------------------
    setup_logging(LOG_DIR, f"v8x_{feat_tag}_adapter_train")

    # -- Load frozen V8.3 base model -------------------------------------------
    base_ckpt_path = BASE_MODEL_DIR / f"v8_3_{feat_tag}_wm_best_ema.pt"
    if not base_ckpt_path.exists():
        base_ckpt_path = BASE_MODEL_DIR / f"v8_3_{feat_tag}_wm_best.pt"
    if not base_ckpt_path.exists():
        base_ckpt_path = BASE_MODEL_DIR / f"v8_3_{feat_tag}_wm_weights.pt"
    if not base_ckpt_path.exists():
        print(f"\n  [ERROR] No V8.3 checkpoint found in {BASE_MODEL_DIR}")
        print("  Train V8.3 first: python src/wm/v8/v8_training/train_world_model.py")
        teardown_logging()
        return

    print(f"\n  Loading base model from {base_ckpt_path.name}")
    base_model = NeuralODEWorldModel(input_dim=input_dim, base_dim=base_dim).to(device_obj)
    ...
    if isinstance(state, dict) and "revin_state_dict" in state:
        ...
        print(f"  RevIN loaded from V8.3.1 checkpoint")
    else:
        print(f"  [WARN] No revin_state_dict in V8.3 checkpoint -- using freshly initialized RevIN")
    revin.eval()
    ...
    adapter_ckpt_path = ADAPTER_MODEL_DIR / f"v8_3_{feat_tag}_adapter_latest.pt"
    start_epoch = 0
    ...
    for epoch in range(start_epoch, ADAPTER_EPOCHS):
        ...
        for step in pbar:
            ...
            # RevIN normalization (frozen, matches V8 training distribution)
            obs_batch = revin(obs_batch, mode='norm')

            # Run frozen V8 forward (no gradients on base model)
            with torch.no_grad():
                base_out = base_model.forward_train(obs_batch, asset_batch)

            # Get V8's combined representation and trunk
        ...
        if (epoch + 1) % ADAPTER_VAL_EVERY == 0 or epoch == ADAPTER_EPOCHS - 1:
            ...
            if val_results["ic_delta_mean"] > best_ic_delta:
                ...
                best_state = {
                    ...
                    "version": "v8x_film_adapter_v2",
                }
                torch.save(best_state, ADAPTER_MODEL_DIR / f"v8_3_{feat_tag}_adapter_best.pt")
                save_marker = " *BEST*"
            ...
            torch.save(ckpt_state, ADAPTER_MODEL_DIR / f"v8_3_{feat_tag}_adapter_latest.pt")
    ...
    # -- Final summary --------------------------------------------------------
    print("\n" + "=" * 70)
    print("  V8.3.X ADAPTER TRAINING COMPLETE")
    print("=" * 70)
    print(f"  Best IC delta:       {best_ic_delta:+.4f}")
    print(f"  Adapter params:      {adapter_params:,}")
    print(f"  Base model hash:     {base_hash}")
    print(f"  Adapter checkpoint:  {ADAPTER_MODEL_DIR / f'v8_3_{feat_tag}_adapter_latest.pt'}")
    print(f"  Best adapter:        {ADAPTER_MODEL_DIR / f'v8_3_{feat_tag}_adapter_best.pt'}")
...
if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description="V8.x Adapter Trainer")

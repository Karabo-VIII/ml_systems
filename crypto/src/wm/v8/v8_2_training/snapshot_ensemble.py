"""
V8.E Seed Ensemble -- Inference wrapper for multi-seed ensemble models.
...
V8-specific:
  - Uses NeuralODEWorldModel (Neural ODE with RK4 solver, RSSM latent space)
  - encode_sequence returns (h_seq, z_post, return_preds) -- RSSM-style
  - model_type="rssm" -- same interface as V1
"""
import torch
...
class SnapshotEnsemble(nn.Module):
    ...
    All models share the same architecture (NeuralODEWorldModel),
    ...
    def __init__(self, snapshot_paths: list = None, top_k: int = ENSEMBLE_TOP_K,
        ...
        if snapshot_paths is None:
            ...
            snapshot_paths = sorted(snap_dir.glob("v8_2e_seed_*_best.pt"))
            if not snapshot_paths:
                # Fallback to final models (backward compatibility)
                snapshot_paths = sorted(snap_dir.glob("v8_2_seed_*.pt"))
    ...
    def get_loss(self, obs_seq, asset_id, target_returns, mask_ratio=0.0, block_mask=False):
        ...
        loss_dict["kl_raw"] = 0.0       # Not meaningful for ensemble
        loss_dict["regime"] = 0.0       # Not computed for ensemble
        loss_dict["regime_acc"] = 0.0   # Not computed for ensemble
        loss_dict["dynamics_reg"] = 0.0 # Not computed for ensemble

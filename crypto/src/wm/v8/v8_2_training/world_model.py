"""
V8.2 World Model -- Neural ODE with Continuous-Time Dynamics + XD Anti-Memorization
...
try:
    from components import TwoHotSymlog, SwiGLU, MLPHead, ODEDynamics, RK4Solver, EulerSolver, RMSNorm
    from settings import *
except ImportError:
    from v8_2_training.components import TwoHotSymlog, SwiGLU, MLPHead, ODEDynamics, RK4Solver, EulerSolver, RMSNorm
    from v8_2_training.settings import *


class NeuralODEWorldModel(nn.Module):
    """
    The V8.2 World Model -- Neural ODE with continuous-time market dynamics + XD Anti-Memorization.
    ...
    def __init__(
        ...
        # == Neural ODE Core (V8.2: Euler solver when ODE_METHOD="euler") ======
        self.dynamics = ODEDynamics(d_model, ode_hidden_layers, dropout, input_dim)
        _ode_method = globals().get("ODE_METHOD", "rk4")
        if _ode_method == "euler":
            self.solver = EulerSolver(self.dynamics, substeps=ODE_SUBSTEPS)
        else:
    ...
    def forward_train(
        ...
        # 3. Prepare obs for ODE conditioning — reads MASKED input_obs.
        # Fixed 2026-05-21 RED-team audit (block-mask was near no-op).
        ...
        # 6. RSSM: Prior + Posterior reads MASKED input_obs base features.
...
if __name__ == "__main__":
    ...
    print(f"V8.2 Neural ODE World Model Parameters: {count_parameters(model):,}")
    ...
    print("V8.2 Neural ODE world model sanity check passed.")

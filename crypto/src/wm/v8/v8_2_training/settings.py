...
V8.2 Settings -- Neural ODE Euler Solver Variant



Ablation: Euler integration (1 eval/step) vs RK4 (4 evals/step).

Cheaper per-step computation. Compensated with more substeps (4 vs 2)

to maintain accuracy. Tests solver quality vs compute tradeoff.
...
MODEL_DIR = PROJECT_ROOT / "models" / "wm" / "v8" / "v8_2"

LOG_DIR = PROJECT_ROOT / "logs" / "v8" / "v8_2"
...
ODE_METHOD = "euler"  # V8.2: Euler solver (was rk4)

ODE_STEP_SIZE = 1.0       # Base integration step size (interval between discrete timesteps)

ODE_SUBSTEPS = 4  # V8.2: more substeps to compensate Euler (was 2)           # Sub-steps per interval (effective step = ODE_STEP_SIZE / ODE_SUBSTEPS)
...
try:
    ...
    _assert_canonical(globals(), version_name="v8_2")
except ImportError:

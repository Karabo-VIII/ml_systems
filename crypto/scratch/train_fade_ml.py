"""ML on PERP short-fade signals (ret_d>+5%, hold 5d)."""
...
for a in assets:
    ...
    d['asset']=a
    signals = d[d['ret_d'] > 0.05].copy()
    if len(signals)==0: continue
    fr = []
    for i in signals.index:
        if i+5 >= len(d): fr.append(np.nan)
        else:
            fr.append((d.iloc[i]['close']/d.iloc[i+5]['close']-1)*100)  # short
    signals['forward_ret_5d_short'] = fr
    signals['net_pnl'] = signals['forward_ret_5d_short'] - 0.08
    signals['label'] = (signals['net_pnl']>0.5).astype(int)
    all_rows.append(signals)

all_sig = pd.concat(all_rows, ignore_index=True).dropna(subset=['forward_ret_5d_short'])
all_sig['date'] = pd.to_datetime(all_sig['date'])
print(f"Total PERP-short signals: {len(all_sig)}")
...
for g in [0.0,0.40,0.45,0.50,0.55,0.60,0.65]:
...
(OUT/'fade_ml_v1.pkl').write_bytes(pickle.dumps({'clf':clf,'features':feat_list}))
print(f"\nSaved to {OUT/'fade_ml_v1.pkl'}")

"""Test 6 exits on TOP robust setups (fixed setup × all 186 mover days) @ 30m."""
import json
...
DATA = ROOT / "runs/mover_day/pepe_train_mover_day_30m_2026_05_27.json"
...
ts_30m = df_30m["timestamp"].to_numpy()
closes_30m = df_30m["close"].to_numpy()
opens_30m = df_30m["open"].to_numpy()
highs_30m = df_30m["high"].to_numpy()
n_train_30m = int(d["_meta"]["n_train_30m_bars"])
...
train_closes = closes_30m[:n_train_30m]
# Top robust setups + 18/42 (analog of 1h 9/21)
SETUPS = [
    ("SMA", 24, 30),    # best median capture
    ("WMA", 6, 24),     # 100% fire, 5.29% med
    ("SMA", 18, 42),    # analog of 1h 9/21, best win rate at high fire
    ("SMA", 30, 42),    # 78.14% win
    ("SMA", 6, 24),     # 100% fire
]

CAP = 14 * 48  # 14 days = 672 30m bars
EXIT_HORIZONS = {"E2_6h": 12, "E3_12h": 24, "E4_24h": 48, "E5_48h": 96}
...
for (kind, fast, slow) in SETUPS:
    ...
    n_entries = 0
    for m in d["movers"]:
        day_close_idx = int(np.searchsorted(ts_30m, m["ts_ms"], side="right") - 1)
        if day_close_idx < 0 or day_close_idx >= n_train_30m:
            continue
        ...
        if opens_30m[ei] <= 0:
            continue
        ep = opens_30m[ei]; n_entries += 1

        # E1: opposite cross
        e1_ex = None
        for i in range(ei, min(ei + CAP, n_train_30m)):
            if not np.isnan(f_ma[i]) and not np.isnan(s_ma[i]) and f_ma[i] <= s_ma[i]:
                e1_ex = i
                break
        if e1_ex is not None and e1_ex + 1 < n_train_30m:
            exit_pnls["E1_opp_cross"].append(opens_30m[e1_ex+1]/ep - 1)

        # E2-E5 fixed holds
        for key, k_bars in EXIT_HORIZONS.items():
            if ei + k_bars < n_train_30m:
                exit_pnls[key].append(opens_30m[ei + k_bars]/ep - 1)

        # E6 MFE-trail50
        peak_p = ep; e6 = None
        end_walk = min(ei + CAP, n_train_30m - 1)
        for i in range(ei + 1, end_walk + 1):
            ...
            if peak_ret >= 0.03:
                ...
                if closes_30m[i] <= lock_p:
                    if i + 1 < n_train_30m:
                        e6 = opens_30m[i+1]/ep - 1
                    else:
                        e6 = closes_30m[i]/ep - 1
                    break
        if e6 is None:
            e6 = closes_30m[end_walk]/ep - 1
...
OUT = ROOT / "runs/mover_day/pepe_train_mover_day_30m_exits_2026_05_27.json"

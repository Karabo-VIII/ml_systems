"""Robustness of fixed setups across all mover days @ 30m cadence."""
import json
...
# Top 15 most-common best-per-day setups + a "deploy candidate analog" for 30m
...
for s, _ in setup_counter.most_common(15):
    CANDIDATES.append(s)
# 30m analog of 1h SMA 9/21: at 30m, 9/21 1h bars ≈ 18/42 30m bars. Add as comparison.
if ("SMA", 18, 42) not in CANDIDATES:
    CANDIDATES.append(("SMA", 18, 42))
...
print(f"\n=== Robustness: fixed setups across all {N_MOVERS} mover days @ 30m ===")
...
for (kind, fast, slow) in CANDIDATES:
    ...
    print(f"{setup_str:>14s} {n_fires:>8d} {100*n_fires/N_MOVERS:>9.1f}% "
          ...
          f"{(caps_arr>0).mean():>8.2%} {np.median(offsets_bars)*0.5:>11.1f}h")
    robust_results[setup_str] = {
        ...
        "median_offset_h_approx": float(np.median(offsets_bars))*0.5,
...
OUT = ROOT / "runs/mover_day/pepe_train_mover_day_30m_robust_2026_05_27.json"
